...
const GAME_VERSION = "V0.9.8.5 平衡与炼蛊";
...
function createLayer3MapState(theme) {
  ...
  /* 段3：绝域残卷（用 reward 类型，enterMapNode 走专门入口） */
  const seg3 = [
    { id: "l3-3-a", step: REST_ROUTE_STEP, type: "reward", layer3: true, l3theme: theme,
      icon: "卷", name: theme === "bone" ? "骨阶残卷" : "蜂蜡残卷", description: LAYER3_NODE_TEXT.reward },
  ];
  /* 段4：第三层 Boss */
  const seg4 = [
    { id: "l3-4-boss", step: BOSS_ROUTE_STEP, type: "boss", enemyId: pool.boss, layer3: true, l3theme: theme,
...
function startFloorBattle() {
  ...
  let musicScene;
  let musicDuration = isBossNode ? 600 : 520;
  ...
  if (runState.layer3?.active) {
    ...
    musicScene = isBossNode ? "boss" : "battle";
  }
...
function finishBattle(victory) {
  ...
  if (victory) {
    ...
    // TODO: 后续多幕路线扩展时抽象 finalNode / bossNode。
    // V0.9.8.5（1-A）：所有层的精英战胜利都走 openCardReward→选牌→蛊炉路径（关炉后 advanceToNextFloor 7696+ 已按层自动回对应层地图）。须置于 layer3/layer2 早返回之前，否则二/三层精英会被提前 return 回地图、永不开炉。
    if (runState.currentNode?.type === "elite") {
