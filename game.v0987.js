...
/* ---- 骨塔高陵路线（护甲/蓄力打断/召卫/执令） ---- */
  ...
  skeletonPuppetGu: {
    ...
    actions: {
      ...
      braceBone: { name: "缩骨蓄甲", icon: "甲", kind: "charge", bonus: 4, armor: 5 },
  ...
  boneArmorGuardGu: {
    ...
    caption: "骨甲蛊卫 · 甲坚击重",
    def: { boneArmorBonus: 4 },
    actions: {
      guardStrike: { name: "甲拳重击", icon: "拳", kind: "attack", damage: 8 },
      plateBrace: { name: "覆甲固身", icon: "覆", kind: "charge", bonus: 4, armor: 8 },
...
function resolveAttack(card, baseDamage, detail = "") {
  ...
  const damage = drunk > 0 ? Math.round(modifiedBase * Math.pow(2, drunk)) : modifiedBase;
