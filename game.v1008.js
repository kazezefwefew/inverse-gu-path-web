...
function refreshModalLock() {
  const hasModal = [dom.resultOverlay, dom.deckOverlay, dom.tutorialOverlay, dom.loreOverlay, dom.balanceOverlay, dom.runStatsOverlay, dom.trialSettingsOverlay, dom.settingsOverlay, dom.overwriteConfirmOverlay] // V0.9.8.7：覆盖确认弹窗纳入模态锁
...
function hideRewardPanels() {
  ...
  dom.shopCloseButton?.classList.add("hidden"); // V0.9.9.1b 蛊坊叉号：非蛊坊态隐藏
...
function completeCurrentNodeAndReturnMap() {
  ...
  if (node.step === 2 && !runState.bossPrepRelicGranted) {
    const relicId = gainRandomOrdinaryRelic("命途整备"); // V0.9.8.6：6段后 step2 不再是 Boss 前，措辞改「命途整备」（仅文案，逻辑不动）
    runState.bossPrepRelicGranted = true;
    if (relicId) {
      runState.lastMapNotice = `${runState.lastMapNotice}；命途整备获得${ORDINARY_RELICS[relicId].name}`;
    }
  }
...
function openShopNode() {
  ...
  dom.resultHp.textContent = runState.currentHp;
  dom.shopPanel.classList.remove("hidden");
  dom.shopCloseButton?.classList.remove("hidden"); // V0.9.9.1b 蛊坊右上角常驻叉号，滚动卡住也能退
  renderShop();
...
function renderGuList(catId) {
  ...
  const isOpen = (it) => (isEco || isHero) ? true : (isBestiary ? (it.enemyId ? bestiary.has(it.enemyId) : true) : isGuUnlocked(it, discovered));
  let head = "";
  let shown = items;
  if (isHero) {
  ...
  shown.forEach((it) => {
    if (isOpen(it)) {
      const pendingFace = (isEco || isBestiary || isHero) ? false : true;
      const face = it.image
...
function resolveAttack(card, baseDamage, detail = "") {
  ...
  const damage = (baseMultiplier > 1 ? Math.round(modifiedBase * baseMultiplier) : modifiedBase) + lifespanBonus;
...
function finishBattle(victory) {
  ...
  if (victory) {
    ...
    if (runState.currentNode?.type === "elite") {
      ...
      const defyRelic = gainRandomOrdinaryRelic("逆命战利品");
...
function openCardReward() {
  ...
  if (runState.currentNode?.type === "elite" || runState.currentNode?.type === "defy") {
    ...
    dom.resultDescription.textContent = `已获得 ${runState.lastBattleRewards?.stones ?? (__defy ? 24 : 16)} 蛊石${material ? `、${material.name}` : ""}${relic ? `与遗物「${relic.name}」` : ""}；${__defy ? "三枚蛊卵中必有稀有，" : ""}选牌后将获得一次炼蛊机会。`;
  } else {
...
function bindEvents() {
  ...
  // V0.9.9.1b 蛊坊右上角叉号 = 离开蛊坊（滚动卡住也能退出）
