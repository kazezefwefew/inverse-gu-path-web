...
const GAME_VERSION = "V0.9.46 残卷急修";
...
const TUTORIAL_PAGES = Object.freeze([
  ...
  },
  {
    ...
    lines: [
      ...
      "烬脉龙蛊达到真形后，烬鳞每局可在焚脉腾渊（化龙额外真元）与玄甲镇脉（化龙立即得甲）中选择一条路线。",
...
function cacheDom() {
  [
    ...
    "guluOverlay", "guluCloseButton", "guluTitle", "guluBody", "guluOpenButton", "baigushiOpenButton", "collectionBadge", "guluBadge", "guluActionConfirm", "guluActionConfirmClose", "guluActionConfirmTitle", "guluActionConfirmText", "guluActionConfirmCancel", "guluActionConfirmOk", // V0.9.22/F-0/P71 蛊庐与局外收藏
    ...
    "battleBackgroundSlot", "battleIntentRegion", "intentBox", "intentIcon", "intentName", "intentDescription", "enemyCriticalMetrics", "intentSummary", "intentCollapseButton", "intentDetailButton", "fateRewriteButton", "fateRewriteChoice", "poisonBorrowButton", "mupanSealPanel", "enemyPower", "enemyStatusList", "arenaKicker",
...
function isAppViewportSamplePlausible(sample, lastGood = null) {
  ...
  if (sameOrientation && comparableWidth && heightCollapsed) return false;
...
function getDeckEntryCost(entry) {
  ...
  const zeroCostAtTwo = entry.key === "wineWorm" || entry.key === "returnBreath";
  const upgradedCost = zeroCostAtTwo && getUpgradeLevel(entry) >= 2 ? 0 : definition.cost || 0;
...
function getCardValues(cardOrKey, forcedLevel = null) {
  ...
  switch (key) {
    ...
    case "returnBreath": return { draw: 2 + (level >= 1 ? 1 : 0), cost: level >= 2 ? 0 : 1 };
    ...
    case "borrowLife": return { selfDamage: Math.max(1, 2 - (level >= 2 ? 1 : 0)), energy: 1, draw: 1 + (level >= 1 ? 1 : 0) };
...
function getCardEffect(key, upgradeLevel = 0) {
  ...
  switch (key) {
    ...
    case "returnBreath": return `抽 <em>${v.draw}</em> 张牌`;
    ...
    case "boneMolt": return `消耗 <em>${v.scaleCost}</em> 枚龙鳞，抽 <em>${v.draw}</em> 张牌并获得 <em>${v.armor}</em> 点防御`;
...
let __lastHarvestSnapshot = null; // AD-1b「看广告翻倍收获」：本局入库量快照（showRunConclusion 写、翻倍按钮读；发过一次即清）

/* AD-1b：看广告完整观看后，把本局入库蛊庐的材料/残核/市契/道行照数再发一遍（翻倍）。
 * 纯局外奖励，零战斗平衡影响；发一次即清快照，防重复领取。返回本次翻倍的文字摘要供提示。 */
function grantDoubledHarvest() {
  const snap = __lastHarvestSnapshot;
  __lastHarvestSnapshot = null; // 先清，杜绝重复领取
  if (!snap) return "";
  ...
  if (snap.scrip > 0 && store.market) { store.market.scrip = (store.market.scrip | 0) + snap.scrip; parts.push(`市契×${snap.scrip}`); }
  ...
  if (parts.length) {
    guluPushEvent(store, `广告加持·收获翻倍：${parts.join("、")}。`);
    saveGuluStore();
    addLogToChannel("journey", `看广告翻倍：${parts.join("、")}已再入蛊庐。`, "positive-log");
  }
  return parts.join("、");
}
  ...
  if (gained > 0) {
    ...
    if (before < DRAGON_BALANCE.scaleMax
      ...
      if (readyArmor > 0) {
        ...
        gainArmor(readyArmor, "烬脉龙蛊", "六鳞初次俱醒");
      }
      window.AudioManager?.playSfx?.("dragonScaleReady", { volumeScale: 0.72 });
      setBattleMessage("六鳞俱醒，点击下方「化龙」即可显露真形。", "important");
    }
  }
  return gained;
}
...
function advanceDragonTransformTurn() {
  ...
  setBattleMessage("龙形已尽，需重新在攻守之间养出六鳞。");
}
...
function activateDragonTransform() {
  ...
  addLog(`烬鳞引六鳞归骨，显露龙形：持续 ${DRAGON_BALANCE.transformTurns} 回合，真元 +1，攻击与防御蛊 +3。`, "important");
...
function renderTowerReflectionScene() {
  ...
  dom.towerHeartBody.textContent = "万命母盘缓缓转动，逐一回放你此行最常使用的战斗方式。";
  ...
  dom.towerHeartFoot.textContent = `最常行为「${snapshot.primary.name}」 · 次常行为「${snapshot.secondary.name}」——由本局行迹推得，读档重算不变。`;
}
...
function renderTowerBossScene() {
  ...
  dom.towerHeartBody.textContent = `盘心已经看穿你的两种习惯——\n最常「${primaryName}」 · 次常「${secondaryName}」。\n触发它正在盯防的行为，追击会在你的回合中立即落下。`;
...
function requestFateRewrite() {
  ...
  if (isMupanBattle()) {
    ...
    if (benmingPassive("fate", 5)) {
      game.inputLocked = true;
      addLog("噬签改命·归墟：已看见夺息刻，可选择采用或保留原技能；母盘的看穿仍会继续。", "important");
      render();
      return true;
    }
...
function getCardBlockReason(card) {
  ...
  if (card.key === "boneMolt" && (!isDragonHero() || game.dragon.transformed || game.dragon.scale < values.scaleCost)) {
...
function classifyAutoPerformance(signals) {
  ...
  const capable = deviceMemory >= 6 || hardwareConcurrency >= 8;
  return {
    mode: "full",
    reason: capable ? "设备能力提示充足，等待短帧率校准" : "设备信息不足，等待短帧率校准",
    frameMs: 0,
    sampleFrames: true,
  };
}
...
const UPDATE_LOG = [
  ...
  { v: "V0.9.45", title: "烬脉龙蛊 · 古鼎开炉", notes: [
    ...
    "战后炼材新增锐骨晶、寿烬与元髓露；百蛊市奇物行扩至八件实用货品，护命蛊匣上限提高至 2，只能用闯塔结算所得市契换取。",
...
function resolveCard(card) {
  ...
  switch (card.key) {
    ...
    case "returnBreath":
      drawCards(v.draw);
      addLog(`回息蛊调匀气脉，抽 ${v.draw} 张牌。`, "positive-log");
      break;
    case "hiddenMeridian":
      gainArmor(v.armor, card.name);
    ...
    case "chiBreath":
      ...
      break;
    case "boneMolt":
      if (spendDragonScale(v.scaleCost, card.name)) {
        drawCards(v.draw);
...
function resolveAttack(card, baseDamage, detail = "") {
  ...
  const modifiedBase = baseDamage + game.attackBonus + bloodBonus + instanceBonus + boneVenomBonus + benmingVenomBonus + carriedAtkBonus + getDragonAttackBonus();
...
function getRunStatsHtml() {
  ...
  const mechanicItems = [
    ...
    ["龙鳞获得 / 龙化次数 / 龙形回合", `${stats.dragonScalesGained || 0} / ${stats.dragonTransforms || 0} / ${stats.dragonTransformTurns || 0}`],
    ...
    ...(stats.mupan ? [
      ["母盘看穿行为", `${MUPAN_DEBT_DEFINITIONS[stats.mupan.primary]?.name || "未记"} / ${MUPAN_DEBT_DEFINITIONS[stats.mupan.secondary]?.name || "未记"}`],
      ["母盘看穿次数", stats.mupan.reads || 0],
      ["成功避开 / 即时追击", `${stats.mupan.successfulBreaks || 0} / ${stats.mupan.pursuits || 0}`],
      ["灭命一击", stats.mupan.finalBlows || 0],
    ] : []),
  ];
  return `
...
function getRunStatsCopyText(outcome = runState?.status) {
  ...
  return [
    ...
    ...(stats.mupan ? [
      `母盘看穿行为：${MUPAN_DEBT_DEFINITIONS[stats.mupan.primary]?.name || "未记"} / ${MUPAN_DEBT_DEFINITIONS[stats.mupan.secondary]?.name || "未记"}`,
      `母盘看穿次数 / 成功避开 / 即时追击 / 灭命一击：${stats.mupan.reads || 0} / ${stats.mupan.successfulBreaks || 0} / ${stats.mupan.pursuits || 0} / ${stats.mupan.finalBlows || 0}`,
      `母盘战斗回合：${stats.mupan.turns || 0}`,
    ] : []),
...
function getFeedbackInfoText(outcome = runState?.status) {
  ...
  return [
    ...
    getAppViewportDebugText(),
    getPerformanceDebugText(),
    `最近运行错误：${lastRuntimeDiagnostic ? `${lastRuntimeDiagnostic.source} · ${safeFeedbackText(lastRuntimeDiagnostic.message)} · ${lastRuntimeDiagnostic.at}` : "无"}`,
    `模式：${getTrialModeInfo(runState.trialMode).name}`,
    `本局模式：${getRunModeLabel(runState.mode)}`,
...
function showRunConclusion(outcome) {
  ...
  // 蛊庐：完整通关或阶段收手全额带出，陨落材料折四成且残核散逸；主动放弃不进本段。
  {
    ...
    const scripSettlement = settleMarketScripFromRun(guluStore, finalGuStones, normalized);
    if (scripSettlement.gained > 0) {
      const capText = scripSettlement.capped ? "（已达本局兑换上限）" : "";
      guluPushEvent(guluStore, `离塔兑契：蛊石 ${scripSettlement.spentStones} 换得市契 ${scripSettlement.gained}${capText}。`);
      addLogToChannel("journey", `百蛊市：蛊石 ${scripSettlement.spentStones} 换得市契 ${scripSettlement.gained}${capText}。`, "positive-log");
      takenParts.push(`市契×${scripSettlement.gained}`);
    ...
    if (Array.isArray(runState.carriedGuIds) && runState.carriedGuIds.length) {
      ...
      if (policy.keepCarriedGu) {
        ...
        if (guSettlement.wardConsumed && guSettlement.preserved.length) {
          guluPushEvent(guluStore, `护命蛊匣碎裂，天品随行蛊「${guSettlement.preserved.join("」「")}」得以归圃。`);
  ...
  dom.runSummary.innerHTML = `
    ...
    ${(__benmingGain || __carryTaken.length || __guSettlement?.wardConsumed) ? `<section class="run-summary-section run-sec-harvest">
      ...
      ${__guSettlement?.wardConsumed ? `<p class="run-sec-sub run-sec-ward-save">护命蛊匣已碎，天品随行蛊「${__guSettlement.preserved.join("」「")}」已安全归圃。</p>` : ""}
      ${dead ? `<p class="run-sec-sub run-sec-harvest-hope">陨落非空手——这些收获已存进蛊庐，下一局更强。</p>` : ""}
      ${(__lastHarvestSnapshot && typeof NmgAds !== "undefined" && NmgAds.isRewardedAvailable()) ? `<button type="button" id="resultRewardedDouble" class="run-rewarded-btn"><strong>看广告 · 本局收获翻倍</strong><small>观看激励视频，蛊庐入库的材料与道行再来一份</small></button>` : ""}
    </section>` : ""}
  ...
  if (isDragonHero()) {
    ...
    buffs.unshift({
      ...
      title: game.dragon.transformed
        ? "龙化期间每回合真元 +1，攻击蛊伤害 +3，护甲蛊防御 +3；期间不能继续获得龙鳞。"
        : "每回合首次以攻击蛊实际伤敌、首次以护甲蛊实际获得防御，各获得 1 枚龙鳞；满 6 枚可主动化龙。",
...
function renderMobileBuffRail(buffs) {
  ...
  const statusMarkup = buffs.map((buff) => {
    ...
    const detail = buff.title || KEYWORD_HELP[buff.keyword] || `${buff.label}。`;
    return `<span class="mobile-status-sigil ${buff.className || ""}"${statusDetailAttr(sigil.detailKey, detail)} title="${escapeAttribute(detail)}"><span aria-hidden="true">${escapeAttribute(sigil.glyph)}</span>${sigil.badge ? `<strong aria-hidden="true">${escapeAttribute(sigil.badge)}</strong>` : ""}</span>`;
  }).join("");
  ...
  const dragonAction = dragonReady
    ? '<button class="dragon-transform-action" type="button" aria-label="消耗六枚龙鳞，显露龙形"><span>化龙</span><strong>六鳞俱醒</strong></button>'
    : "";
  dom.mobileBuffRail.innerHTML = relicMarkup + statusMarkup + dragonAction;
...
function statusDetailAttr(keyword, fallback) {
...
function enemyStatusAttr(keyword, fallback) {
  return statusDetailAttr(keyword, fallback);
}
...
function renderEnemyStatuses() {
  ...
  if (isMupanBattle()) {
    ...
    if (game.mupan.exposureActive) statuses.push(`<span class="enemy-status enemy-charge-status">盘心暴露 · 承伤 +35%</span>`);
  }
  if (game.enemy.id === "corpsepuppet") {
...
function updateIntentDetailAccess(action) {
  if (!dom.intentDetailButton || !action) return;
  ...
  const detail = parts.join("；") || "敌人将在你结束回合后施展此招。";
  ENEMY_STATUS_HELP[keyword] = detail;
  dom.intentDetailButton.dataset.keyword = keyword;
  dom.intentDetailButton.title = `${action.name}：${detail}`;
  dom.intentDetailButton.setAttribute("aria-label", `查看${action.name}完整说明：${detail}`);
}
...
function bindEvents() {
  ...
  dom.guluOverlay?.addEventListener("click", (event) => {
    ...
    if (guluTab) {
      ...
      guluActiveTab = nextGuluTab === "collection" ? nextGuluTab : "home";
      renderGulu();
      return;
    }
    ...
    if (marketStall) {
      ...
      if (["insects", "materials", "curios", "ward"].includes(stall)) guluMarketStall = stall;
      renderGulu();
      return;
    }
    ...
    if (featuredEgg && !featuredEgg.disabled) {
      playGuluClick();
      const result = buyBaigushiFeaturedEgg();
      guluNoticeText = result.text;
      if (result.ok) window.AudioManager?.playSfx?.("guluPot", { volumeScale: 0.9 });
      renderGulu();
      return;
    }
    const incenseTarget = event.target.closest("[data-baigushi-incense-target]");
    if (incenseTarget && !incenseTarget.disabled) {
      playGuluClick();
      const result = buyBaigushiGrowthIncense(incenseTarget.dataset.baigushiIncenseTarget);
      guluNoticeText = result.text;
      if (result.ok) window.AudioManager?.playSfx?.("guluFeed", { volumeScale: 0.75 });
      renderGulu();
      return;
    }
    ...
    if (healingSalve && !healingSalve.disabled) {
      playGuluClick();
      const result = buyBaigushiHealingSalve();
      guluNoticeText = result.text;
    ...
    if (materialCrate && !materialCrate.disabled) {
      playGuluClick();
      const result = buyBaigushiMaterialCrate();
      guluNoticeText = result.text;
      if (result.ok) window.AudioManager?.playSfx?.("guluPot", { volumeScale: 0.82 });
      renderGulu();
      return;
    }
    const choiceMaterial = event.target.closest("[data-baigushi-choice-material]");
    if (choiceMaterial && !choiceMaterial.disabled) {
      playGuluClick();
      const result = buyBaigushiChoiceMaterials(choiceMaterial.dataset.baigushiChoiceMaterial);
      guluNoticeText = result.text;
      if (result.ok) window.AudioManager?.playSfx?.("guluPot", { volumeScale: 0.82 });
      renderGulu();
      return;
    ...
    if (gradeEgg && !gradeEgg.disabled) {
      playGuluClick();
      const result = buyBaigushiGradeSeal(gradeEgg.dataset.baigushiGradeEgg);
      guluNoticeText = result.text;
    ...
    if (marrowGu && !marrowGu.disabled) {
      playGuluClick();
      const result = buyBaigushiMarrowJade(marrowGu.dataset.baigushiMarrowGu);
      guluNoticeText = result.text;
    ...
    const coreCrate = event.target.closest("[data-baigushi-core-crate]");
    if (coreCrate && !coreCrate.disabled) {
      playGuluClick();
      const result = buyBaigushiCoreCrate();
      guluNoticeText = result.text;
    ...
    if (marketRecipe && !marketRecipe.disabled) {
      playGuluClick();
      const result = buyBaigushiRecipe(marketRecipe.dataset.baigushiRecipe);
      guluNoticeText = result.text;
      if (result.ok) window.AudioManager?.playSfx?.("guluPot", { volumeScale: 0.9 });
      renderGulu();
      return;
    ...
    if (marketWard && !marketWard.disabled) {
      playGuluClick();
      const result = buyBaigushiDeathWard();
      guluNoticeText = result.text;
      if (result.ok) window.AudioManager?.playSfx?.("guluPot", { volumeScale: 0.95 });
      renderGulu();
      return;
    ...
    if (marketMaterial && !marketMaterial.disabled) {
      playGuluClick();
      const result = buyBaigushiMaterial(marketMaterial.dataset.baigushiMaterial);
      guluNoticeText = result.text;
  ...
  dom.runSummary?.addEventListener("click", async (event) => {
    // AD-1b：看广告 · 本局收获翻倍（玩家自愿点，看完 isEnded 才发奖，仅一次）
    ...
    if (adBtn) {
      ...
      NmgAds.showRewarded((ok) => {
        if (ok) {
          const summary = grantDoubledHarvest();
          adBtn.classList.add("is-claimed");
          adBtn.setAttribute("disabled", "true");
          if (label) label.textContent = summary ? `已翻倍：${summary}` : "收获已翻倍";
        } else {
          adBtn.dataset.busy = "";
...
document.addEventListener("DOMContentLoaded", async () => {
  cacheDom();
  initCloudSaveUi();
  try { await window.NMGCloudSave.bootstrap(); } catch (e) { /* 云端失败必须放行本地游戏 */ }
