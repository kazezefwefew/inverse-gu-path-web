...
// V0.9.69：六流派 PVE 首轮基准与两项定向补强。
// 这里只记录可复测的开局底线和封顶规则；其余流派没有证据时不顺手改数值。
const PVE_FLOW_BALANCE = Object.freeze({
  fixedSeeds: Object.freeze([11, 29, 47, 83, 131, 197]),
  starterBaselines: Object.freeze({
    fate: Object.freeze({
      protectionKeys: Object.freeze(["ironSkin", "reversePath"]),
      engineKeys: Object.freeze(["fateThread", "reversePath"]),
    }),
    blood: Object.freeze({
      protectionKeys: Object.freeze(["ironSkin", "bloodThirst"]),
      engineKeys: Object.freeze(["bloodBlade", "bloodSacrifice", "bloodTide"]),
    }),
    poison: Object.freeze({
      protectionKeys: Object.freeze(["ironSkin", "moltingShell"]),
      engineKeys: Object.freeze(["greenMiasma", "insectSwarm", "poisonReturn"]),
    }),
    longevity: Object.freeze({
      protectionKeys: Object.freeze(["ironSkin", "witheredBloom"]),
      engineKeys: Object.freeze(["lifeFlame", "soulCrack"]),
    }),
    dragon: Object.freeze({
      protectionKeys: Object.freeze(["ironSkin", "scaleHiding", "boneMolt"]),
      engineKeys: Object.freeze(["scaleHiding", "reverseScale", "cloudHorn"]),
    }),
    bone: Object.freeze({
      protectionKeys: Object.freeze(["ironSkin", "boneBell", "boneCourt"]),
      engineKeys: Object.freeze(["boneBell", "knockArmor", "breakJoint"]),
    }),
  }),
  bloodBladeRecovery: Object.freeze({ baseHeal: 1, bloodPerHeal: 2, healCap: 4 }),
  poisonShellDefense: Object.freeze({ poisonPerArmor: 2, armorCap: 6 }),
  fateCoinBattleStoneCap: 3,
});

function planFateCoinBattleReward(currentStones, cap = PVE_FLOW_BALANCE.fateCoinBattleStoneCap) {
  const current = Math.max(0, Math.floor(Number(currentStones) || 0));
  const limit = Math.max(0, Math.floor(Number(cap) || 0));
  const stones = current < limit ? 1 : 0;
  return Object.freeze({ stones, next: current + stones, capped: stones === 0, cap: limit });
}

function planBloodBladeRecovery(bloodSnapshot, damageDealt, rule = PVE_FLOW_BALANCE.bloodBladeRecovery) {
  const safeBlood = Math.max(0, Math.floor(Number(bloodSnapshot) || 0));
  const safeDamage = Math.max(0, Math.floor(Number(damageDealt) || 0));
  const healed = safeDamage > 0
    ? Math.min(rule.healCap, rule.baseHeal + Math.floor(safeBlood / rule.bloodPerHeal))
    : 0;
  return Object.freeze({ bloodSnapshot: safeBlood, damageDealt: safeDamage, healed });
}

function planPoisonShellDefense(baseArmor, poisonSnapshot, rule = PVE_FLOW_BALANCE.poisonShellDefense) {
  const safeBaseArmor = Math.max(0, Math.floor(Number(baseArmor) || 0));
  const safePoison = Math.max(0, Math.floor(Number(poisonSnapshot) || 0));
  const poisonArmor = Math.min(rule.armorCap, Math.floor(safePoison / rule.poisonPerArmor));
  return Object.freeze({
    baseArmor: safeBaseArmor,
    poisonSnapshot: safePoison,
    poisonArmor,
    totalArmor: safeBaseArmor + poisonArmor,
  });
}
...
const ACTION_ECONOMY_PROGRESSION = Object.freeze({
  borrowTideSilkworm: Object.freeze([
    { draw: 1, discard: 0, costReduction: 0, energyDebt: 1, waiveHighCost: false, flowArmor: 2, discardGuard: 0, cost: 0 },
    { draw: 1, discard: 0, costReduction: 0, energyDebt: 1, waiveHighCost: false, flowArmor: 3, discardGuard: 0, cost: 0 },
    { draw: 1, discard: 0, costReduction: 1, energyDebt: 1, waiveHighCost: false, flowArmor: 4, discardGuard: 0, cost: 0 },
    { draw: 1, discard: 0, costReduction: 1, energyDebt: 1, waiveHighCost: false, flowArmor: 5, discardGuard: 0, cost: 0 },
    { draw: 1, discard: 0, costReduction: 1, energyDebt: 1, waiveHighCost: false, flowArmor: 6, discardGuard: 0, cost: 0 },
    { draw: 2, discard: 1, costReduction: 1, energyDebt: 1, waiveHighCost: false, flowArmor: 4, discardGuard: 3, cost: 0 },
    { draw: 2, discard: 1, costReduction: 1, energyDebt: 1, waiveHighCost: false, flowArmor: 4, discardGuard: 4, cost: 0 },
    { draw: 2, discard: 1, costReduction: 1, energyDebt: 1, waiveHighCost: false, flowArmor: 4, discardGuard: 5, cost: 0 },
    { draw: 2, discard: 1, costReduction: 1, energyDebt: 1, waiveHighCost: true, flowArmor: 4, discardGuard: 6, cost: 0 },
  ].map(Object.freeze)),
  ...
  returnBreath: Object.freeze([
    { draw: 2, discard: 0, armor: 0, cost: 1 },
    { draw: 2, discard: 0, armor: 2, cost: 1 },
    { draw: 3, discard: 0, armor: 2, cost: 1 },
    { draw: 3, discard: 0, armor: 4, cost: 1 },
    { draw: 3, discard: 0, armor: 6, cost: 1 },
  ...
  redTideBladeLeech: Object.freeze([
    { damage: 5, bloodCost: 2, bloodCap: 3, perBlood: 4, ecologyBonus: 3, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 6, bloodCost: 2, bloodCap: 3, perBlood: 4, ecologyBonus: 3, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 7, bloodCost: 2, bloodCap: 4, perBlood: 4, ecologyBonus: 4, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 8, bloodCost: 2, bloodCap: 4, perBlood: 5, ecologyBonus: 4, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 9, bloodCost: 2, bloodCap: 5, perBlood: 5, ecologyBonus: 5, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 10, bloodCost: 2, bloodCap: 5, perBlood: 5, ecologyBonus: 6, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 11, bloodCost: 2, bloodCap: 5, perBlood: 6, ecologyBonus: 6, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 12, bloodCost: 2, bloodCap: 6, perBlood: 6, ecologyBonus: 7, selfDamage: 3, bloodGain: 1, cost: 1 },
    { damage: 14, bloodCost: 2, bloodCap: 6, perBlood: 7, ecologyBonus: 8, selfDamage: 3, bloodGain: 1, cost: 1 },
  ].map(Object.freeze)),
  ...
  witheredMulberryTurtle: Object.freeze([
    { armor: 7, lifespanCost: 1, weaken: 3, weakenCap: 3, perWeakenArmor: 2, armorScaleCap: 2, ecologyArmorRemove: 4, cost: 2 },
    { armor: 9, lifespanCost: 1, weaken: 3, weakenCap: 3, perWeakenArmor: 2, armorScaleCap: 2, ecologyArmorRemove: 4, cost: 2 },
    { armor: 11, lifespanCost: 1, weaken: 3, weakenCap: 3, perWeakenArmor: 2, armorScaleCap: 3, ecologyArmorRemove: 5, cost: 2 },
    { armor: 13, lifespanCost: 1, weaken: 4, weakenCap: 4, perWeakenArmor: 3, armorScaleCap: 3, ecologyArmorRemove: 5, cost: 2 },
    { armor: 15, lifespanCost: 1, weaken: 4, weakenCap: 4, perWeakenArmor: 3, armorScaleCap: 3, ecologyArmorRemove: 6, cost: 2 },
    { armor: 17, lifespanCost: 1, weaken: 5, weakenCap: 5, perWeakenArmor: 3, armorScaleCap: 4, ecologyArmorRemove: 7, cost: 2 },
    { armor: 19, lifespanCost: 1, weaken: 5, weakenCap: 5, perWeakenArmor: 4, armorScaleCap: 4, ecologyArmorRemove: 8, cost: 2 },
    { armor: 21, lifespanCost: 1, weaken: 5, weakenCap: 5, perWeakenArmor: 4, armorScaleCap: 4, ecologyArmorRemove: 9, cost: 2 },
    { armor: 23, lifespanCost: 1, weaken: 6, weakenCap: 6, perWeakenArmor: 5, armorScaleCap: 5, ecologyArmorRemove: 10, cost: 2 },
  ].map(Object.freeze)),
});
...
const RESOURCE_SAFE_EXEMPTIONS = Object.freeze({
  sequenceCicada: Object.freeze({ role: "support", reason: "九转抽牌与换类护甲由 ACTION_ECONOMY_PROGRESSION 明确列出；不额外叠加真元收益" }),
  borrowTideSilkworm: Object.freeze({ role: "support", reason: "九转换拍、潮护、抽弃与有限减费由 ACTION_ECONOMY_PROGRESSION 明确列出；次回合真元债始终为 1" }),
  ...
  returnBreath: Object.freeze({ role: "support", reason: "九转抽牌、防御与首次额外抽牌由 ACTION_ECONOMY_PROGRESSION 明确列出；稳定定位不再附带弃牌" }),
  fateMeasureInchworm: Object.freeze({ role: "support", reason: "珍稀蛊九转只在完成攻、防、辅三类记录后一次性抽1并结甲；每战一次且不提供真元净收益" }),
  emberRemnant: Object.freeze({ role: "defense-support", reason: "抽牌三转封顶；弃牌成甲已在 getCardBaseValues 采用 8+4L 主曲线，不再叠加封顶后成长" }),
...
/* V0.9.71：事件选择跨战回响。数值只在此处定义，规则模块、UI 与结算均读取同一份配置。 */
const PVE_CAUSAL_BALANCE = Object.freeze({
  modePressure: Object.freeze({
    normal: Object.freeze({ intensity: 1, maxPrimary: 1, maxSecondary: 0 }),
    elite: Object.freeze({ intensity: 1.15, maxPrimary: 1, maxSecondary: 1 }),
    compatibleSecondaries: Object.freeze({
      poison: Object.freeze(["armor"]),
      blood: Object.freeze(["lifesteal"]),
      bone: Object.freeze(["interrupt"]),
      swarm: Object.freeze(["card_count_pressure"]),
    }),
  }),
  expansionMechanics: Object.freeze({
    minimumCounterDamage: 1,
    maximumStacks: 3,
    stackAttackBonus: 2,
    markedAttackBonus: 3,
    counterAttackPenalty: 3,
  }),
  eventEchoes: Object.freeze({
    restlessEgg: Object.freeze({ openingHpLoss: 4, winMaterialId: "insectMolt", winMaterialAmount: 1 }),
    steleTrial: Object.freeze({ winStones: 4 }),
    caravanPursuit: Object.freeze({ enemyAttackBonus: 2, winStones: 5 }),
    poisonShadow: Object.freeze({ openingHpLoss: 3, firstDamagePoison: 3 }),
  }),
  enemyLessons: Object.freeze({
    rewardStones: 3,
    shanxiaoInterruptThreshold: 8,
    skeletonInterruptThreshold: 13,
    bloodGapDamageMultiplier: 1.25,
    bloodGapMaxBonusDamage: 6,
    poisonWindowTurns: 2,
    slowHandMaxCards: 4,
    slowHandTurns: 2,
  }),
