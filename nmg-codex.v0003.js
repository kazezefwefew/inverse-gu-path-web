...
function renderBenmingCodex() {
  ...
  const cards = Object.entries(BENMING_GU).map(([heroId, gu]) => {
    ...
    const pathOverview = heroId === "fate" ? `<div class="benming-path-overview ${info.stage < 3 ? "is-locked" : ""}">
      <p><strong>真形双路线</strong><span>${legacyRun ? "当前老续局沿用旧规则" : (info.stage >= 3 ? "每局开局二择一" : "真形后解锁")}</span></p>
      ${Object.values(BENMING_PATHS.fate).map((path) => `<section class="benming-path-entry ${selectedPathId === path.id ? "is-active" : ""}">
