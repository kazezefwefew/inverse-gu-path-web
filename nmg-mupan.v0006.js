...
function getMupanIntentDamage(action = {}, { attackMultiplier = 1, rageBonus = 0, weaken = 0, attackBonusMultiplier = 1 } = {}) {
  ...
  const segments = fixed
    ? action.fixedSegments.map((value) => Math.max(0, Math.round((getMupanNonNegative(value) + getMupanNonNegative(rageBonus)) * multiplier)))
    : Array.from({ length: hits }, () => Math.max(0, Math.round((getMupanNonNegative(action.damage) + getMupanNonNegative(rageBonus)) * multiplier)));
  const rawTotal = segments.reduce((sum, value) => sum + value, 0);
  const weakenCut = fixed ? 0 : Math.min(getMupanNonNegative(weaken), Math.floor(rawTotal * 0.5));
  let remainingCut = weakenCut;
  for (let index = 0; index < segments.length && remainingCut > 0; index += 1) {
    const cut = Math.min(segments[index], remainingCut);
    segments[index] -= cut;
    remainingCut -= cut;
  }
  const total = segments.reduce((sum, value) => sum + value, 0);
  return { perHit: segments[0] || 0, hits: segments.length, segments, total, weakenCut };
}
