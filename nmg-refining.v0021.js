...
function resolveMaterialReward(materialId = null) {
  ...
  if (materialId) {
    ...
    runState.firstRewardClaimedPart = true;
    if (runState.firstRunGuard) runState.firstRunGuard.rewardClaimedPart = true;
...
function renderFurnaceChoice(entry) {
  ...
  const currentTerms = getCardRefineTerms(entry);
  const capTerms = getCardRefineTerms({ ...entry, upgradeLevel: getUpgradeCapFor(entry) });
  ...
  return `<button class="furnace-choice ${level > 0 ? `upgraded upgrade-${level}` : ""} ${entry.mutated ? "is-mutated" : ""}" type="button" data-furnace-card="${entry.instanceId}">
    <span class="upgrade-badge">${level > 0 || currentTerms.kind === "carried-turn" ? currentTerms.label : "未炼化 · 可炼化"}</span>
    ...
    <dl class="furnace-card-meta">
      ...
      <div><dt>上限</dt><dd>${level >= getUpgradeCapFor(entry) ? `已至${currentTerms.label}` : `可炼至${capTerms.label}`}</dd></div>
...
function getUpgradeResultText(entry, oldLevel, newLevel) {
  ...
  switch (entry.key) {
    ...
    case "bloodBlade": return `炼成${name}，基础伤害提升至 ${v.damage}；血煞收益与实伤后的封顶回血保留。`;
    ...
    case "mysticCarapace":
    case "fixedFate": return `炼成${name}，基础防御提升至 ${v.armor}。`;
    case "moltingShell": return `炼成${name}，基础防御提升至 ${v.armor}；毒转甲上限 ${v.armorCap} 点并保留中毒抽牌。`;
...
function applyMutationFurnace(entry, materialId) {
  ...
  entry.mutated = true;
  if (typeof markGuDiscovered === "function") markGuDiscovered(targetKey);
