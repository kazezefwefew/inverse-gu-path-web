...
(function createNmgVoiceDirector(global) {
  ...
  const VOICE_ASSET_MANIFEST = Object.freeze([
    ...
    "assets/audio/voices/wenling/select.mp3", "assets/audio/voices/wenling/start.mp3", "assets/audio/voices/wenling/battle.mp3", "assets/audio/voices/wenling/elite.mp3", "assets/audio/voices/wenling/boss.mp3", "assets/audio/voices/wenling/chime_01.mp3", "assets/audio/voices/wenling/chime_02.mp3", "assets/audio/voices/wenling/lowlife.mp3", "assets/audio/voices/wenling/restore.mp3", "assets/audio/voices/wenling/victory.mp3", "assets/audio/voices/wenling/bosswin.mp3", "assets/audio/voices/wenling/defeat.mp3", "assets/audio/voices/wenling/ending.mp3",
  ]);
  ...
  const folderVoice = (prefix, file, priority) => Object.freeze({ key: `${prefix}_${file}`, src: `assets/audio/voices/${prefix}/${file}.mp3`, priority });
  const folderHeroVoices = (prefix, coreEvent, coreOne, coreTwo) => {
    const entries = {
      select: folderVoice(prefix, "select", 40), start: folderVoice(prefix, "start", 40),
      battle: folderVoice(prefix, "battle", 50), elite: folderVoice(prefix, "elite", 50), boss: folderVoice(prefix, "boss", 80),
      [coreOne]: folderVoice(prefix, coreOne, 70), [coreTwo]: folderVoice(prefix, coreTwo, 70),
      lowlife: folderVoice(prefix, "lowlife", 70), restore: folderVoice(prefix, "restore", 10), victory: folderVoice(prefix, "victory", 60),
      bosswin: folderVoice(prefix, "bosswin", 80), defeat: folderVoice(prefix, "defeat", 90), ending: folderVoice(prefix, "ending", 100),
    };
    Object.defineProperty(entries, "coreEvent", { value: coreEvent, enumerable: false });
    Object.defineProperty(entries, "bossWin", { value: entries.bosswin, enumerable: false });
    return Object.freeze(entries);
  ...
  const HERO_VOICES = Object.freeze({
    ...
    bone: folderHeroVoices("wenling", "chime", "chime_01", "chime_02"),
  });
