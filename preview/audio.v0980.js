...
(function createAudioManager(global) {
  ...
  const SFX = Object.freeze({
    ...
    guluPot: "assets/audio/gulu/gulu-pot.v1.mp3",
  ...
  const AMBIENTS = Object.freeze({
    guluNight: { src: "assets/audio/gulu/gulu-night-insects.v1.mp3", baseVolume: 0.5 },
  });
  ...
  function applyMuteState() {
    ...
    if (ambientAudio) { ambientAudio.muted = muted; if (muted) ambientAudio.pause(); else if (ambientKey && !document.hidden) ambientAudio.play().catch(() => {}); } // V0.9.26 环境层同守开关（关=真静音停播）
  }
  ...
  function playAmbient(key, { fadeMs = 2000 } = {}) {
    ...
    a.muted = muted;
    a.play().catch(() => { /* 播放策略拒绝时静默，蛊庐内必然已有用户手势，正常不会触发 */ });
  ...
  function resumeBgm() {
    ...
    if (ambientAudio && ambientKey) ambientAudio.play().catch(() => {}); // V0.9.26 环境层随前台恢复
    const scene = resolveDesiredScene();
    if (!scene) return;
    lifecyclePaused = false;
