...
const GAME_VERSION = "V0.9.7.1 PWA安装（preview）";
...
const DEV_ACTIONS = {
  ...
  // —— PWA 检查（V0.9.7.1）——
  "pwaLogState": devPwaLogState,
  "pwaClearCache": devPwaClearCache,
  "pwaUnregister": devPwaUnregisterSw,
  "pwaReload": devPwaReload,
  "pwaShowGuide": devPwaShowGuide,
};
...
const DEV_PANEL_GROUPS = [
  ...
  { title: "PWA检查", buttons: [
    ["打印PWA状态", "pwaLogState"], ["清PWA缓存", "pwaClearCache"],
    ["注销Service Worker", "pwaUnregister"], ["重载页面", "pwaReload"],
    ["打开安装说明", "pwaShowGuide"],
  ] },
];
...
function bootLoaderEnter() {
  ...
  bootLoaderHide();
  // V0.9.7.1：入局淡出后挂一次性「可添加到桌面」轻提示（不挡菜单、不影响 BGM）
  try { window.setTimeout(maybePwaInstallHint, 900); } catch (e) {}
}
...
function initDevMode() {
  ...
  buildDevPanelDom();
  try { buildDevPwaStatePanel(); } catch (e) {}
  console.log("[DEV] 开发者测试模式已启用（preview + dev=kaan）。");
}
/* =================== /DEV MODE =================== */
/* ===================== V0.9.7.1 PWA 安装 UI / 检测 / 轻提示（纯加性，绝不改音频/战斗/boot-loader 主流程） =====================
   - window.__pwaState 单一真相源；若 index.html 早期脚本已建则复用并补字段。
   - 安装按钮永不死：A standalone 不显 / B beforeinstallprompt 安装 / C 查看安装说明 / D 内置浏览器提示。
   - 安装说明弹窗按设备分文案；轻提示一次性（localStorage 记关闭）。
   - 全程 try/catch，任何 PWA 能力缺失都不影响游戏。 */

function pwaState() {
  if (typeof window === "undefined") return null;
  if (!window.__pwaState || typeof window.__pwaState !== "object") {
    window.__pwaState = {};
  }
  return window.__pwaState;
}

function pwaDetectStandalone() {
  try {
...
function pwaDetectInApp() {
  try {
    const ua = (navigator.userAgent || "");
    return /MicroMessenger|QQ\/|Qzone|QQBrowser|DouYin|TikTok|musical_ly|Weibo|WeiBo|Baiduboxapp|Alipay|UCBrowser/i.test(ua);
  } catch (e) { return false; }
}

/* 设备/浏览器粗分类，供安装说明弹窗选文案与按钮分流。 */
function pwaDetectPlatform() {
  let ua = "";
  try { ua = navigator.userAgent || ""; } catch (e) {}
  const isInApp = pwaDetectInApp();
  ...
  // iOS 上的 Chrome/Edge/Firefox（非 Safari 内核能力）
  const isIOSNonSafari = isIOS && /CriOS|FxiOS|EdgiOS/i.test(ua);
  let key = "desktop";
  if (isInApp) key = "inapp";
  else if (isIOS && (isSafari || (!isIOSNonSafari && !isInApp))) key = "ios-safari";
  else if (isIOSNonSafari) key = "ios-other";
  else if (isAndroid) key = "android";
  else key = "desktop";
  return { ua, isInApp, isIOS, isAndroid, isEdge, isChrome, isSafari, isIOSNonSafari, key };
}

/* 计算安装按钮应处的状态：installed / install / guide / inapp / hidden */
function pwaComputeButtonState() {
  const st = pwaState();
  if (!st) return "hidden";
  if (st.isStandalone) return "installed";
  if (st.beforeInstallPromptCaptured && st.deferredPrompt) return "install";
  if (st.isInAppBrowser) return "inapp";
  return "guide"; // 永不死：兜底永远是「查看安装说明」
}

/* 初始化 / 刷新 __pwaState（幂等，可多次调用）。console 输出关键键便于调试。 */
function initPwaState() {
  const st = pwaState();
  if (!st) return;
  st.isStandalone = pwaDetectStandalone();
  st.isInAppBrowser = pwaDetectInApp();
  if (typeof st.manifestLoaded !== "boolean") {
    try { st.manifestLoaded = !!document.querySelector('link[rel="manifest"]'); } catch (e) { st.manifestLoaded = false; }
  }
  if (typeof st.serviceWorkerRegistered !== "boolean") st.serviceWorkerRegistered = false;
  if (typeof st.beforeInstallPromptCaptured !== "boolean") st.beforeInstallPromptCaptured = false;
  if (!("deferredPrompt" in st)) st.deferredPrompt = null;
  if (!st.cacheVersion) st.cacheVersion = "nmg-pwa-v0971";
  try { st.startUrl = location.href; } catch (e) {}
  if (!st.scope) st.scope = "./";
  st.canInstall = !!(st.beforeInstallPromptCaptured && st.deferredPrompt);
  st.buttonState = pwaComputeButtonState();
  try {
    console.log("[PWA] state:", {
      isStandalone: st.isStandalone,
      isInAppBrowser: st.isInAppBrowser,
      manifestLoaded: st.manifestLoaded,
      serviceWorkerRegistered: st.serviceWorkerRegistered,
      beforeInstallPromptCaptured: st.beforeInstallPromptCaptured,
      canInstall: st.canInstall,
      buttonState: st.buttonState,
      cacheVersion: st.cacheVersion,
...
/* beforeinstallprompt / appinstalled 监听（幂等，标记防重复绑定）。
   注意：index.html 早期脚本可能已捕获 deferredPrompt，这里只补绑一次以防早期脚本缺失。 */
let __pwaListenersBound = false;
function bindPwaEvents() {
  if (__pwaListenersBound) return;
  __pwaListenersBound = true;
  try {
    window.addEventListener("beforeinstallprompt", (e) => {
      // 若早期脚本已捕获则不重复 preventDefault，仅同步状态，避免逻辑歧义。
      const st0 = pwaState();
      if (!(st0 && st0.beforeInstallPromptCaptured && st0.deferredPrompt)) {
        try { e.preventDefault(); } catch (err) {}
      }
      const st = pwaState();
      if (!st) return;
      st.deferredPrompt = e;
      st.beforeInstallPromptCaptured = true;
      st.canInstall = true;
      st.buttonState = "install";
    ...
    window.addEventListener("appinstalled", () => {
      ...
      if (st) {
        ...
        st.buttonState = "installed";
      }
      console.log("[PWA] appinstalled 已安装");
      pwaShowHintToast("已添加到主屏幕");
      updatePwaButtonState();
    });
    // standalone 状态变化（部分浏览器支持）
    try {
      const mq = window.matchMedia && window.matchMedia("(display-mode: standalone)");
      if (mq && mq.addEventListener) {
        mq.addEventListener("change", () => { initPwaState(); updatePwaButtonState(); });
      }
    } catch (e) {}
    // 自定义事件：index.html 早期捕获 beforeinstallprompt 后派发
    document.addEventListener("pwa:install-ready", () => { initPwaState(); updatePwaButtonState(); });
    // index.html 早期脚本派发的「已安装」事件 → 可见反馈
    document.addEventListener("pwa:installed", () => {
      const st = pwaState();
      if (st) { st.isStandalone = true; st.deferredPrompt = null; st.canInstall = false; st.buttonState = "installed"; }
      pwaShowHintToast("已添加到主屏幕");
      updatePwaButtonState();
    });
    // SW 新版本就绪 → 战斗中只提示不强刷，非战斗可提示刷新载入
    document.addEventListener("pwa:update-ready", (e) => {
      let playing = false;
      try { playing = !!(e && e.detail && e.detail.playing); } catch (er) {}
      if (!playing) {
        try { playing = (typeof game !== "undefined" && game && game.status === "playing"); } catch (er) {}
      }
      if (playing) pwaShowHintToast("发现新版本残卷，战斗结束后刷新生效");
      else pwaShowHintToast("发现新版本残卷，可刷新载入");
    });
  } catch (e) { console.warn("[PWA] 事件绑定失败", e); }
}

/* ---------- 安装按钮（注入菜单 .start-guide-actions，4 态分流） ---------- */
function ensurePwaInstallButton() {
  let btn = document.getElementById("pwaMenuInstallButton");
  if (btn) return btn;
  const actions = document.querySelector(".start-guide-actions");
  if (!actions) return null;
  btn = document.createElement("button");
  btn.id = "pwaMenuInstallButton";
  btn.type = "button";
  btn.className = "start-guide-button pwa-menu-install-button";
  btn.textContent = "添加到桌面";
  btn.addEventListener("click", onPwaInstallButtonClick);
  // 放在「设置」按钮之后较自然，找不到就追加到末尾前的重置按钮前
  const resetBtn = actions.querySelector("#tutorialResetButton");
...
function updatePwaButtonState() {
  ...
  if (!st) return;
  st.buttonState = pwaComputeButtonState();
  const btn = document.getElementById("pwaMenuInstallButton");
  if (!btn) return;
  switch (st.buttonState) {
    case "installed":
      // 已 standalone：不显安装按钮（保持菜单整洁）
...
function onPwaInstallButtonClick() {
  const st = pwaState();
  if (!st) { pwaOpenInstallGuide(); return; }
  const state = pwaComputeButtonState();
  if (state === "install" && st.deferredPrompt) {
      ...
      if (uc && typeof uc.then === "function") {
        uc.then((choice) => {
          if (choice && choice.outcome === "accepted") {
            pwaShowHintToast("已添加到桌面");
          } else {
            pwaShowHintToast("可稍后在「设置」中重新安装");
          }
          st.deferredPrompt = null;
          st.canInstall = false;
          st.beforeInstallPromptCaptured = false;
          updatePwaButtonState();
        }).catch(() => { st.deferredPrompt = null; updatePwaButtonState(); });
      }
    } catch (e) {
      // 原生框失败也兜底打开说明，绝不无响应
      pwaOpenInstallGuide();
  ...
  // inapp / guide / 其它一律打开说明弹窗（永不无响应）
  pwaOpenInstallGuide();
}

/* ---------- 安装说明弹窗（按设备分文案） ---------- */
function pwaGuideContentFor(platform) {
  const big = '<p class="pwa-guide-big">';
  switch (platform.key) {
    case "android":
      return (
        '<p class="pwa-guide-lead">安卓 · Chrome / Edge</p>' +
        big + '若下方或地址栏出现「安装应用 / 添加到主屏幕」提示，直接点它即可。</p>' +
        '<ol class="pwa-guide-steps">' +
        '<li>点击浏览器右上角 <b>⋮</b> 菜单。</li>' +
        '<li>选择 <b>「安装应用」</b> 或 <b>「添加到主屏幕」</b>。</li>' +
        '<li>确认后，桌面会出现「逆命蛊途」图标，像 App 一样打开。</li>' +
        '</ol>'
      );
    case "ios-safari":
      return (
        '<p class="pwa-guide-lead">iPhone · Safari</p>' +
        big + 'iPhone 添加到主屏幕需用 <b>Safari</b> 自带的「分享」。</p>' +
        '<ol class="pwa-guide-steps">' +
        '<li>点击底部中间的 <b>分享按钮</b>（方框带向上箭头）。</li>' +
        '<li>在列表中向下找到并点击 <b>「添加到主屏幕」</b>。</li>' +
        '<li>右上角点 <b>「添加」</b>，主屏幕即出现图标。</li>' +
        '</ol>'
      );
    case "ios-other":
      return (
        '<p class="pwa-guide-lead">iPhone · Chrome / Edge / 其它</p>' +
        big + 'iPhone 上只有 <b>Safari</b> 能添加到主屏幕。</p>' +
        '<ol class="pwa-guide-steps">' +
        '<li>复制当前网址，或在 <b>Safari</b> 中重新打开本页。</li>' +
        '<li>点底部 <b>分享按钮</b> →「添加到主屏幕」→「添加」。</li>' +
        '</ol>'
      );
    case "inapp":
      return (
        '<p class="pwa-guide-lead">微信 / QQ / 抖音 等内置浏览器</p>' +
        big + '当前是 App 内置浏览器，<b>不能直接添加到主屏幕</b>。</p>' +
        '<ol class="pwa-guide-steps">' +
        '<li>点击右上角 <b>⋯</b>，选择 <b>「在浏览器打开」</b>。</li>' +
        '<li>安卓用 <b>Chrome</b>、iPhone 用 <b>Safari</b> 打开本页。</li>' +
        '<li>再按对应步骤添加到主屏幕。</li>' +
        '</ol>'
      );
    case "desktop":
    default:
      return (
        '<p class="pwa-guide-lead">电脑 · Chrome / Edge</p>' +
        big + '可把本游戏安装为桌面应用，独立窗口打开。</p>' +
        '<ol class="pwa-guide-steps">' +
        '<li>看地址栏右侧是否有 <b>安装图标</b>（屏幕带下箭头），点它即可。</li>' +
        '<li>或点右上角 <b>⋮</b> 菜单 → <b>「安装 逆命蛊途…」</b>。</li>' +
        '</ol>'
      );
  }
}

function pwaOpenInstallGuide() {
  let overlay = document.getElementById("pwaInstallGuideOverlay");
  const platform = pwaDetectPlatform();
  const inner = pwaGuideContentFor(platform);
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.id = "pwaInstallGuideOverlay";
    overlay.className = "pwa-install-overlay hidden";
    overlay.setAttribute("role", "dialog");
    overlay.setAttribute("aria-modal", "true");
    overlay.setAttribute("aria-label", "添加到主屏幕说明");
    overlay.innerHTML =
      '<div class="pwa-install-card">' +
      '<button type="button" class="pwa-install-close" id="pwaInstallGuideClose" aria-label="关闭">×</button>' +
      '<h2 class="pwa-install-title">添加到主屏幕</h2>' +
      '<div class="pwa-install-body" id="pwaInstallGuideBody"></div>' +
      '<p class="pwa-install-foot">添加后可像 App 一样全屏打开，无需每次输入网址。</p>' +
      '<button type="button" class="pwa-install-ok" id="pwaInstallGuideOk">我知道了</button>' +
      '</div>';
    document.body.appendChild(overlay);
    const close = () => { overlay.classList.add("hidden"); };
    const closeBtn = overlay.querySelector("#pwaInstallGuideClose");
    const okBtn = overlay.querySelector("#pwaInstallGuideOk");
    if (closeBtn) closeBtn.addEventListener("click", close);
    if (okBtn) okBtn.addEventListener("click", close);
    overlay.addEventListener("click", (e) => { if (e.target === overlay) close(); });
  }
  const body = overlay.querySelector("#pwaInstallGuideBody");
  if (body) body.innerHTML = inner;
  overlay.classList.remove("hidden");
}

/* ---------- 启动后一次性轻提示 ---------- */
function pwaShowHintToast(message, opts) {
  opts = opts || {};
  try {
    let toast = document.getElementById("pwaHintToast");
    if (!toast) {
      toast = document.createElement("div");
      toast.id = "pwaHintToast";
      toast.className = "pwa-hint-toast hidden";
      toast.setAttribute("role", "status");
      toast.innerHTML =
        '<span class="pwa-hint-text"></span>' +
        '<button type="button" class="pwa-hint-close" aria-label="关闭轻提示">×</button>';
      document.body.appendChild(toast);
      const closeBtn = toast.querySelector(".pwa-hint-close");
      if (closeBtn) closeBtn.addEventListener("click", () => {
        toast.classList.add("hidden");
        if (toast.dataset.persistKey) {
          try { localStorage.setItem(toast.dataset.persistKey, "1"); } catch (e) {}
        }
      });
    }
    const textEl = toast.querySelector(".pwa-hint-text");
    if (textEl) textEl.textContent = message;
    toast.dataset.persistKey = opts.persistKey || "";
    // 安装按钮内置浏览器/普通提示：可点提示打开说明
    toast.onclick = opts.openGuideOnClick ? (e) => {
      if (e.target.classList && e.target.classList.contains("pwa-hint-close")) return;
      pwaOpenInstallGuide();
    } : null;
    toast.classList.remove("hidden");
    if (!opts.persistKey) {
      // 临时提示自动消失
      window.setTimeout(() => { toast.classList.add("hidden"); }, opts.duration || 2600);
    }
  } catch (e) { console.warn("[PWA] toast 失败", e); }
}

/* boot-loader 入局后调用一次：非 standalone 且未关闭过 → 显示「可添加到手机桌面」。
   不挡菜单按钮（fixed 角标）、不影响 BGM。 */
function maybePwaInstallHint() {
  try {
    const st = pwaState();
    if (!st || st.isStandalone) return;
    if (localStorage.getItem("nmg.pwa.hintDismissed") === "1") return;
    pwaShowHintToast("可添加到手机桌面，像 App 一样打开", {
      persistKey: "nmg.pwa.hintDismissed",
      openGuideOnClick: true,
    });
  } catch (e) {}
}

/* 主菜单显示时调用：注入按钮 + 刷新状态。 */
function refreshPwaMenuUi() {
  try {
    ensurePwaInstallButton();
    initPwaState();
    updatePwaButtonState();
  } catch (e) { console.warn("[PWA] 菜单 UI 刷新失败", e); }
}

/* 总初始化：DOMContentLoaded 时调用一次。 */
function initPwaUi() {
  try {
    bindPwaEvents();
    initPwaState();
  } catch (e) { console.warn("[PWA] 初始化失败", e); }
}

/* ---------- Dev PWA 检查区（状态显示面板，仅 isDevMode） ---------- */
function buildDevPwaStatePanel() {
  try {
    const host = document.querySelector(".dev-panel-body");
    if (!host) return;
    let block = host.querySelector("#devPwaStatePanel");
    if (!block) {
      block = document.createElement("div");
      block.id = "devPwaStatePanel";
      block.className = "dev-pwa-state";
    ...
    const yn = (v) => (v ? "是" : "否");
    const rows = [
      ["standalone", yn(st.isStandalone)],
      ["SW注册", yn(st.serviceWorkerRegistered)],
      ["beforeinstallprompt", yn(st.beforeInstallPromptCaptured)],
      ["内置浏览器", yn(st.isInAppBrowser)],
      ["manifest加载", yn(st.manifestLoaded)],
      ["start_url", String(st.startUrl || "-")],
      ["scope", String(st.scope || "-")],
      ["缓存版本", String(st.cacheVersion || "-")],
      ["可安装", yn(st.canInstall)],
      ["按钮状态", String(st.buttonState || "-")],
    ];
    block.innerHTML = rows.map(([k, v]) =>
      '<div class="dev-pwa-state-row"><span>' + k + '</span><strong>' + v + '</strong></div>'
    ).join("");
  } catch (e) { console.warn("[PWA] Dev 状态区构建失败", e); }
}

/* ---------- Dev PWA 检查区 actions（供 DEV_ACTIONS 引用） ---------- */
function devPwaLogState() {
  initPwaState();
  console.log("[DEV PWA] __pwaState =", pwaState());
  try { buildDevPwaStatePanel(); } catch (e) {}
  devNotify("已打印 PWA 状态到 console（F12）。", "system-log");
}
function devPwaClearCache() {
  if (!(window.caches && caches.keys)) { devNotify("当前环境无 Cache API。", "damage-log"); return; }
  caches.keys()
    .then((ks) => Promise.all(ks.map((k) => caches.delete(k))))
    .then(() => devNotify("PWA 缓存已清除（caches.delete 全部）。", "positive-log"))
    .catch((e) => { console.warn(e); devNotify("清缓存失败（见 console）。", "damage-log"); });
}
function devPwaUnregisterSw() {
  if (!(navigator.serviceWorker && navigator.serviceWorker.getRegistrations)) { devNotify("当前环境不支持 Service Worker。", "damage-log"); return; }
  navigator.serviceWorker.getRegistrations()
    .then((rs) => Promise.all(rs.map((r) => r.unregister())))
    .then(() => devNotify("Service Worker 已全部注销。", "positive-log"))
    .catch((e) => { console.warn(e); devNotify("注销 SW 失败（见 console）。", "damage-log"); });
}
function devPwaReload() { try { location.reload(); } catch (e) {} }
function devPwaShowGuide() { pwaOpenInstallGuide(); devNotify("已打开安装说明弹窗。", "system-log"); }

/* 跨脚本可调：index.html 早期脚本派发的 pwa:update-ready / 安装反馈可直接调用 window.pwaShowHintToast。 */
try { window.pwaShowHintToast = pwaShowHintToast; window.pwaOpenInstallGuide = pwaOpenInstallGuide; } catch (e) {}
/* ===================== /V0.9.7.1 PWA 安装 UI ===================== */
