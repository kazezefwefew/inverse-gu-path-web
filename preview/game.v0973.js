...
/* ---- 骨塔高陵路线（护甲/蓄力打断/召卫/执令） ---- */
  bonebellGu: {
    name: "骨铃蛊", title: "骨塔杂蛊", maxHp: 44,
    kicker: "骨铃轻摇，乱人心神",
    intro: "骨铃蛊背壳垂着一串森白骨铃，每一次摇动都在你耳中织出杂音。",
    caption: "骨铃蛊 · 乱铃扰神",
    actions: {
  ...
  skeletonPuppetGu: {
    name: "尸傀蛊兵", title: "骨塔列卒", maxHp: 62,
    kicker: "尸傀列阵，蓄力重砸",
    intro: "尸傀蛊兵由残骨与蛊丝拼缀成形，举起骨锤时关节咔咔作响。",
    caption: "尸傀蛊兵 · 蓄力重击",
    actions: {
  ...
  boneNestGuardianBoss: {
    name: "骨巢守墓王", title: "骨塔之主", maxHp: 140, isBoss: true,
    kicker: "骨巢镇陵，守墓不退",
...
/* ---- 蜂窟魔巢路线（毒刺/蜂群/抢攻/多段） ---- */
  ...
  chaosSwarmHordeGu: {
    name: "乱蜂蛊潮", title: "蜂窟潮群", maxHp: 56,
    kicker: "乱蜂成潮，密则反噬",
    intro: "乱蜂蛊潮是无数毒蜂裹成的一团活潮，你出手越密，它越是乱翅抢攻。",
    caption: "乱蜂蛊潮 · 密牌抢攻",
...
const GAME_VERSION = "V0.9.8 第三层生态（preview）";
...
const LAYER3_ROUTES = {
  bone: {
    ...
    enemiesPreview: "可能遭遇：骨铃蛊 · 尸傀蛊兵 · 骨甲蛊卫 · 骨塔执令者 · 骨巢守墓王",
  ...
  beehive: {
    ...
    enemiesPreview: "可能遭遇：毒蜂蛊 · 蜂巢虫蛊 · 乱蜂蛊潮 · 蜂窟守卫 · 灾厄蜂后",
...
function showCommandPathChoice() {
  ...
  dom.resultEyebrow.textContent = "命途更深 · 生态之下";
  dom.resultTitle.textContent = "深泽已破，蛊息未止";
  dom.resultDescription.textContent = "深泽已破，蛊息却未止。远处骨塔有铃声摇动，蜂窟中万翅齐鸣…就此收功，抑或踏入更深的绝域？";
  dom.resultDeckButton?.classList.remove("hidden");
  dom.resultStatsButton?.classList.remove("hidden");
  dom.resultLoreButton?.classList.remove("hidden");
  dom.resultFeedbackButton?.classList.add("hidden");
...
function resolveEnemyTurn() {
  ...
  if (action.kind === "charge") {
    ...
    // V0.9.8 三层·抢攻（乱蜂蛊潮/蜂窟守卫）：本玩家回合出牌数 > counterAttackThreshold（默认 4）则该次攻击 +counterDamage（默认 8）。意图侧已提前预警。
...
function beginNextTurn() {
  ...
  // V0.9.8 三层·乱铃（骨铃蛊）：本回合少抽 nextTurnDrawPenalty 张，保底抽至 1 张，随后清零。
...
function analyzeDeathCause(stats, runState) {
  ...
  // 三层·未打断骨塔蓄力重击
  if (dc.enemyCharge && dc.source === "enemyAttack" && (dc.enemyChargeInterrupt > 0 || enemyName.indexOf("骨巢守墓王") >= 0 || enemyName.indexOf("尸傀蛊兵") >= 0)) return make("未打断骨塔蓄力", "骨塔敌人蓄力重击未被打断，承受了完整一击；蓄力可在受够阈值伤害后打断。");
  // 三层·骨甲过高输出突不破
...
function renderIntent() {
  ...
  if (action.kind === "charge") {
    ...
    const totalDamage = action.damage * hitCount + game.enemy.chargedBonus + lowHpBonus + enrageBonus + routeBonus + mechBonus;
    const extras = [];
