...
const HERO_EXCLUSIVE_CARD_KEYS = Object.freeze({
  ...
  longevity: ["lifeFlame", "witheredBloom", "soulCrack"],
});
...
function resolveAttack(card, baseDamage, detail = "") {
  ...
  // 焚寿燃命加成在酒虫倍率「之后」相加，避免被 ×2^层放大成爆炸数值
  const lifespanBonus = (game.player.heroId === "longevity") ? getLifespanDamageBonus(game.player) : 0;
  const damage = (drunk > 0 ? Math.round(modifiedBase * Math.pow(2, drunk)) : modifiedBase) + lifespanBonus;
...
function renderIntent() {
  ...
  if (action.kind === "charge") {
    ...
    const totalDamage = Math.round(action.damage * hitCount * (game.enemyAttackMultiplier || 1)) + game.enemy.chargedBonus + lowHpBonus + enrageBonus + routeBonus + mechBonus;
    const extras = [];
