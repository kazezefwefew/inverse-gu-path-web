...
let selectedMode = "normal"; // "normal" | "elite"，开局写入 runState.mode
const MODE_TUNING = {
...
function createRunState() {
  ...
  return {
    ...
    relicId,
    trialMode: mode,
    mode: progression.eliteUnlocked ? selectedMode : "normal", // V0.9.8.3：普通/精英，未解锁强制普通
    trialSeed: seed,
    rngState,
    currentHp: maxHp,
    maxHp,
...
function getRunStatsCopyText(cleared = runState?.status === "cleared") {
  ...
  return [
    ...
    `本局模式：${runState.mode === "elite" ? "精英" : "普通"}`,
    `命途种子：${runState.trialSeed || "无"}`,
...
function showRunConclusion(cleared) {
  ...
  dom.runSummary.innerHTML = `
    ...
    <section class="run-summary-section run-sec-result ${cleared ? "run-sec-clear" : "run-sec-death"}">
      ...
      <p class="run-sec-sub">入塔蛊修 ${hero?.name || "未知蛊修"} · 本命遗物 ${relic?.name || "未知遗物"} · ${getTrialModeInfo(runState.trialMode)?.name || "标准试炼"} · 本局模式 <strong>${runState.mode === "elite" ? "精英" : "普通"}</strong></p>
    </section>
