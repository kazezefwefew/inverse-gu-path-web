...
function relicFactionLabel(tone) {
  return ({ jade: "护身", blood: "血道", poison: "毒道", gold: "通用", soul: "燃命" })[tone] || "通用";
}
...
function getRelicCatalogItems() {
  ...
  Object.keys(ORDINARY_RELICS).forEach((key) => {
    ...
    items.push({ id: "relic_" + key, category: "relic", relicId: key, name: r.name, glyph: r.glyph, tone: r.tone || "gold",
      kind: "随身遗物", rarity: "随身", faction: relicFactionLabel(r.tone), type: "遗物", gameplayEffect: r.description, dropsFrom: "命途所得（精英 / 逆命 / 血签 / 命途整备）" });
  });
  return items;
}
