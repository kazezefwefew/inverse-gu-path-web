...
/* ---- 骨塔高陵路线（护甲/蓄力打断/召卫/执令） ---- */
  ...
  skeletonPuppetGu: {
    ...
    actions: {
      ...
      braceBone: { name: "缩骨蓄甲", icon: "甲", kind: "charge", bonus: 4, armor: 8 },
  ...
  boneArmorGuardGu: {
    ...
    actions: {
      ...
      plateBrace: { name: "覆甲固身", icon: "覆", kind: "charge", bonus: 4, armor: 12 },
  ...
  boneNestGuardianBoss: {
    ...
    actions: {
      ...
      sepulchreCharge: { name: "召卫蓄力", icon: "召", kind: "charge", bonus: 9, armor: 10, interruptThreshold: 14 },
    },
...
function pickRandomAvailableRelicId() {
  ...
  const available = ORDINARY_RELIC_IDS.filter((id) => {
    if (hasOrdinaryRelic(id)) return false;
...
function flushPendingRelicOffer() {
  ...
  if (!offer || !ORDINARY_RELICS[offer.relicId] || !dom.relicOfferOverlay) return;
...
function resolveRelicOffer(accept) {
  ...
  if (offer && ORDINARY_RELICS[offer.relicId]) {
    if (accept) gainOrdinaryRelic(offer.relicId, offer.source);
    else addLog(`你舍弃了遗物「${ORDINARY_RELICS[offer.relicId].name}」。`, "system-log");
  }
...
function renderCombatRelicStrip() {
  ...
  dom.combatRelicStrip.innerHTML = ids.map((id) => {
...
function gainGuStones(amount, source = "命途所得") {
  ...
  amount = Math.round(amount * (getModeTuning().rewardMul || 1));
...
function sellShopLifespan() {
  ...
  gainGuStones(LIFE_SELL_STONE_GAIN, "焚寿易石");
...
function resolveEnemyTurn() {
  ...
  if (action.kind === "charge") {
    ...
    if (action.interruptThreshold) {
      game.enemy.charging = true;
      game.enemy.currentInterruptThreshold = action.interruptThreshold;
    }
    ...
    if (action.armor) {
      const armorCap = Number(action.armorCap) || 0;
...
function checkLayer2EnemyPoisonMechanics() {
  ...
  if (def.poisonConvert && game.enemy.poison >= def.poisonConvert.threshold) {
    ...
    if (!onCooldown) {
      ...
      game.player.poison = Math.min(cap, game.player.poison + cv.give);
...
function finishBattle(victory) {
  ...
  if (victory) {
    ...
    if (runState.currentNode?.type === "elite") {
      ...
      gainGuStones(8, "普通战斗胜利");
      runState.lastBattleRewards = { type: "battle", stones: 8, materialId: null, furnace: false };
    }
    // TODO: 后续多幕路线扩展时抽象 finalNode / bossNode。
...
function snapshotDeathContext() {
  ...
  return {
    ...
    enemyChargeInterrupt: Number((typeof game !== "undefined" && game && game.enemy && game.enemy.currentInterruptThreshold) || 0) || 0,
...
function generateRunTitle(stats, runState, cleared) {
  ...
  if (cleared && rs.mode === "deathtrial") return wrap("死劫 · 焚命渡劫者", "九死无生处，你以一身焚烬踏碎死劫——金印加身。");
  const l3 = rs.layer3 || {};
  ...
  if (l3BossDefeated) return wrap("三层踏尽", "骨塔与蜂窟皆已踏尽，绝域之主尽数伏于你的蛊息之下。");
  // ⓪ 败于三层 Boss 留名
  if (deathEnemy.indexOf("骨巢守墓王") >= 0) return wrap("骨巢破封者", "骨巢之巅，你与守墓王同葬于这片万骨高陵。");
  if (deathEnemy.indexOf("灾厄蜂后") >= 0) return wrap("蜂后伏诛者", "魔巢深处，万翅同振，你倒在蜂后的毒潮之中。");
  // ⓪ 三层路线阵亡
  if (inLayer3Death && isBoneRoute) return wrap("骨塔折铃", "森白骨铃乱响，你的命途断折在这座万骨高陵之上。");
  if (inLayer3Death && isBeehiveRoute) return wrap("蜂窟坠命", "毒翅蔽空，你坠入蜂窟魔巢，命数为万蜂所噬。");
  // ① 通关二层
...
function renderIntent() {
  ...
  if (action.kind === "charge") {
    ...
    const rawTotal = Math.round(action.damage * hitCount * (game.enemyAttackMultiplier || 1)) + game.enemy.chargedBonus + lowHpBonus + enrageBonus + routeBonus + mechBonus;
...
function applyUpdateNow() {
  try {
    ...
    location.replace(base + "?_upd=" + Date.now()); // 查询串绕过 index.html 缓存，强制取最新
