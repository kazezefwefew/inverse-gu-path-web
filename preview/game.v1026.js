...
const GAME_VERSION = "V0.9.15 路线残卷与所求";
...
function cacheDom() {
  [
    ...
    "activeRelicName", "combatRelicStrip", "enemySideLabel", "enemyTitle", "enemyHp", "enemyMaxHp", "enemyHpBar", "enemyPortrait",
...
function renderGuList(catId) {
  ...
  const items = isRelic ? getRelicCatalogItems() : (window.GU_CATALOG || []).filter((it) => it.category === catId);
  ...
  const isOpen = (it) => isRelic ? isRelicUnlocked(it, discoveredRelics) : ((isEco || isHero) ? true : (isBestiary ? (it.enemyId ? bestiary.has(it.enemyId) : true) : isGuUnlocked(it, discovered)));
  let head = "";
  let shown = items;
  if (isHero) {
  ...
  shown.forEach((it) => {
    if (isOpen(it)) {
      const pendingFace = (isEco || isBestiary || isHero || isRelic) ? false : true;
      const face = it.image
...
function finishBattle(victory) {
  ...
  if (victory) {
    ...
    if (runState.currentNode?.type === "elite") {
      ...
      runState.lastBattleRewards = { type: "battle", stones: battleStones, materialId: null, furnace: false };
    }
