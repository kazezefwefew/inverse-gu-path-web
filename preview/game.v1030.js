...
// 开场序章：首次进入开始界面自动弹一次，可在设置里重看。呼应残卷「黑石—代价—塔门」神话。
const PROLOGUE_PAGES = Object.freeze([
  { title: "黑石", text: "传说最初之人跪在黑石前，割血三滴，断发一缕，又吹灭半盏寿灯。黑石开口：凡欲改命，须先拿命中之物来喂。\n\n于是世间有了蛊——不是恩赐，是代价开出的路。" },
...
const GAME_VERSION = "V0.9.18 塔中回声";
...
function getModeTuning() { return MODE_TUNING[(runState && runState.mode) || "normal"] || MODE_TUNING.normal; }
// V0.9.9 子批6：模式中文名（三档），供结算/反馈文案统一显示，避免「死劫被显示成普通」。
function getRunModeLabel(mode) { return mode === "deathtrial" ? "死劫" : mode === "elite" ? "精英" : "普通"; }
// V0.9.9 子批6：解析本局生效模式——死劫需已解锁，精英需已解锁，否则降级（防越权/坏档）。
...
function getBloodMax() {
  const base = BLOOD_MAX + Math.max(0, Number(runState?.bloodMaxBonus) || 0);
...
function grantBattleItem(itemId, sourceName = "命途所得") {
  ...
  if (runState.satchel.length >= SATCHEL_CAP) {
    gainGuStones(4, `蛊囊已满，「${item.name}」折算`, { raw: true });
    return false;
  }
  runState.satchel.push(itemId);
  addLog(`${sourceName}：蛊囊收入「${item.name}」——${item.description}`, "positive-log");
  return true;
}
...
/* 战斗内点击蛊囊芯片使用：仅玩家回合（inputLocked 为假）可用；效果全走现有入口。 */
...
/* 蛊囊条：战斗内常驻（空囊隐藏），点击即用；与遗物条同款芯片视觉。 */
...
function createRunState() {
  ...
  const maxLifespan = hero.lifespan + (relicId === "siSuiLun" ? 12 : 0);
  ...
  return {
    ...
    relicId,
    trialMode: mode,
    mode: resolveRunMode(), // V0.9.8.3/子批6：普通/精英/死劫，未解锁自动降级
    trialSeed: seed,
    rngState,
    currentHp: maxHp,
    maxHp,
    ...
    satchel: [], // V0.9.16 蛊囊：战斗消耗品（≤3 格），随局持久
    materialHistory: {},
    nextBattleHpLoss: 0,
...
function renderTitleScreen() {
  dom.heroChoices.innerHTML = Object.entries(HEROES).map(([id, hero]) => {
    ...
    // V0.9.14 蛊修印录：选人卡角标显示该英雄最高通关印记——选人界面就是成就墙
    const bestSeal = getHeroBestSealMode(id);
    const sealBadge = bestSeal ? `<span class="hero-seal-badge ${SEAL_MODE_META[bestSeal].cls}" title="${SEAL_MODE_META[bestSeal].full}">${SEAL_MODE_META[bestSeal].label}</span>` : "";
...
function renderModeChoices() {
  ...
  dom.modeChoices.innerHTML = modes.map((m) => `
...
function maybeMeetSiming() {
  ...
  openSimingEvent(layer);
  return true;
}
function openSimingEvent(layer) {
  runState.simingMetLayers.push(layer);
...
/* V0.9.16 蛊囊：蛊坊丹囊格——每坊一件（进坊时按流派偏发选定），满囊时禁购。 */
const SHOP_ITEM_PRICE = 9;
function renderShopItemTrade(state) {
  const item = BATTLE_ITEMS[state.itemKey];
  if (!item) return "";
  const satchelFull = (runState.satchel || []).length >= SATCHEL_CAP;
  const disabled = state.item || satchelFull || runState.guStones < getShopPrice(SHOP_ITEM_PRICE);
  const reason = state.item ? "已售出" : satchelFull ? "蛊囊已满" : getShopTradeLabel(state.item, getShopPrice(SHOP_ITEM_PRICE));
...
function buyShopItem() {
  const state = getShopState();
  const item = BATTLE_ITEMS[state.itemKey];
  if (!item || state.item || (runState.satchel || []).length >= SATCHEL_CAP) return;
...
const UPDATE_LOG = [
  ...
  { v: "V0.9.16", title: "蛊囊", notes: [
    "新系统「蛊囊」：随身 3 格战斗消耗品——囊中养的活蛊，战斗中点击即用、用一次即失，给你出牌之外的第二个答案。",
    "12 种囊中蛊：通用 4 种（回血/护甲/抽牌/回真元）+ 各流派专属 2 种（如血道炽血露直伤、毒道爆毒囊、命势引势铃、寿道驻颜露），按流派偏发。",
    "获取途径：普通战斗约四分之一掉落、蛊坊新增丹囊格、炼蛊「蛊损」反噬改为补偿一件消耗品（蛊损不再白亏）；蛊囊满时自动折算 4 蛊石。",
    "战斗界面中央新增蛊囊条（手机横屏落在空腰扩展位）；万蛊录新开「丹囊」分类可查全部 12 种。",
...
/* V0.9.16 蛊囊：丹囊图鉴条目（恒显，小池子直接看全，帮助玩家理解新系统）。 */
...
const GU_CATEGORIES = [
  ...
  { id: "item", label: "丹囊", ready: true }, // V0.9.16 蛊囊：战斗消耗品，恒显
...
function renderSealCodex() {
  ...
  const heroRows = Object.entries(HEROES).map(([heroId, hero]) => {
    ...
    return '<div class="seal-hero-row"><span class="seal-hero-name">' + escGu(hero.name) + '</span><span class="seal-chips">' + chips + '</span></div>';
  }).join("");
...
function getLifespanDamageBonus(player) {
  ...
  return hasOrdinaryRelic("soulBurnMirror") ? base * 2 : base; // V0.9.9.2 焚魂镜：焚寿燃命伤害加成翻倍
}
...
function resolveAttack(card, baseDamage, detail = "") {
  ...
  const modifiedBase = baseDamage + game.attackBonus + bloodBonus + instanceBonus + boneVenomBonus;
...
function finishBattle(victory) {
  ...
  if (victory) {
    ...
    if (runState.currentNode?.type === "elite") {
      ...
      // V0.9.16 蛊囊：普通战约 1/4 掉 1 个消耗品（走 reward 种子通道；满囊自动折算蛊石）。
      ...
      if (getRunRandom("reward") < 0.25) {
        ...
        if (dropId && grantBattleItem(dropId, "战利蛊囊")) droppedItemId = dropId;
      }
...
function openCardReward() {
  ...
  if (runState.currentNode?.type === "elite" || runState.currentNode?.type === "defy") {
    ...
    // V0.9.16 蛊囊：普通战掉落的消耗品在结算文案里点名，别让玩家错过
    const __droppedItem = BATTLE_ITEMS[runState.lastBattleRewards?.itemId];
    dom.resultDescription.textContent = `从三枚新生蛊卵中收纳一枚，或舍弃收获继续前行。${__droppedItem ? `蛊囊另收入「${__droppedItem.name}」。` : ""}`;
  }
  dom.cardRewardChoices.innerHTML = choices.map((key) => {
    const item = CARD_LIBRARY[key];
    return `<button class="reward-card" type="button" data-reward-card="${key}">
...
function applyBacklashFurnace(entry) {
  ...
  if (result === "damageCard") {
    ...
    // V0.9.16 蛊囊：蛊损不再白亏——炉中残料化作一件消耗品作补偿（满囊自动折算蛊石）
...
function render() {
  ...
  renderSatchelStrip(); // V0.9.16 蛊囊条
  updateGuStoneDisplays();
...
function bindEvents() {
  ...
  // V0.9.16 蛊囊：点击芯片使用消耗品（委托，索引从 data-satchel-index 取）
  ...
  dom.shopActions?.addEventListener("click", (event) => {
    ...
    else if (action === "item") buyShopItem(); // V0.9.16 蛊囊
...
const DEV_ACTIONS = {
  ...
  "elitePrintMode": () => { const t = MODE_TUNING[(runState && runState.mode) || "normal"]; devNotify(`选择=${selectedMode} 本局=${runState?.mode || "-"} 已解锁=${progression.eliteUnlocked} HP×${t.hpMul}/攻×${t.atkMul}/奖×${t.rewardMul}`, "system-log"); console.log("[DEV] mode", { selectedMode, runMode: runState?.mode, eliteUnlocked: progression.eliteUnlocked, tuning: t }); },
