...
const GAME_VERSION = "V0.9.19a 十重天·批1";
...
const TIAN_TIER_MODS = Object.freeze([
  ...
  { tier: 3, name: "薄囊", desc: "丹囊上限 3→2", live: false },
  { tier: 4, name: "贵市", desc: "蛊坊价格 +25%", live: false },
  { tier: 5, name: "蚀寿", desc: "每过一层焚 1 寿元", live: false },
  { tier: 6, name: "炉险", desc: "炼蛊反噬概率提高", live: false },
  { tier: 7, name: "天妒", desc: "稀有牌出率减半", live: false },
  { tier: 8, name: "孤行", desc: "开局本命遗物三选一→二选一", live: false },
...
function createRunState() {
  ...
  // V0.9.19 十重天：先解析模式与重数（九重·残躯要在算血量上限前生效）。
...
// 更新公告（只记正式版本；最新的放最前）。
const UPDATE_LOG = [
  { v: "V0.9.19a", title: "十重天·第一批（preview）", notes: [
    "新难度「十重天」：死劫金印后解锁的天梯——十重递进，每登一重多压一条修饰，第 N 重通关解锁第 N+1 重，可随时回打低重。",
    "四位蛊修各自爬各自的天梯：进度按英雄独立记录，选人后在模式行查看该蛊修已通至第几重。",
    "本批实装数值四重：一重·塔压加身（敌人生命 +15%）、二重·凶戾（敌人攻击 +10%）、九重·残躯（生命上限 -10%）、十重·逆命天（Boss 血/攻再 +20%，无续局）；奖励随重数上浮（约 1.3→1.8 倍）。",
    "三～八重的规则修饰（薄囊/贵市/蚀寿/炉险/天妒/孤行）已在清单中列出、标注待实装，将于第二批接入。",
    "进塔前所见即所得：模式行选中十重天后，步进条下方直接列出当前重数的全部生效修饰。",
  ] },
