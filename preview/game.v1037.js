...
const GAME_VERSION = "V0.9.20.2 本命蛊";
...
function renderTitleScreen() {
  dom.heroChoices.innerHTML = Object.entries(HEROES).map(([id, hero]) => {
    ...
    // V0.9.9 子批3后修：朝暮立绘是「数组」(4 档随寿元切换)，选将界面取第 1 档(满寿)；否则 ${数组} 会被拼成逗号串成的坏路径→立绘加载失败显占位符。
    const heroPortrait = Array.isArray(PORTRAIT_PATHS.heroes[id]) ? PORTRAIT_PATHS.heroes[id][0] : PORTRAIT_PATHS.heroes[id];
    // V0.9.14 蛊修印录：选人卡角标显示该英雄最高通关印记——选人界面就是成就墙。V0.9.19：天印（十重天进度）盖过金印显示。
    const bestSeal = getHeroBestSealMode(id);
    const tianClearedTier = getTianCleared(id);
    const sealBadge = tianClearedTier > 0
      ? `<span class="hero-seal-badge seal-tian" title="天印 · 十重天已通至第 ${tianClearedTier} 重">天·${tianClearedTier}重</span>`
      : (bestSeal ? `<span class="hero-seal-badge ${SEAL_MODE_META[bestSeal].cls}" title="${SEAL_MODE_META[bestSeal].full}">${SEAL_MODE_META[bestSeal].label}</span>` : "");
    return `<button class="hero-choice hero-${id} ${selected ? "selected" : ""}"
      ...
      <div class="hero-choice-portrait">
        ...
        <img class="hero-choice-image" data-portrait-path="${heroPortrait}" alt="${hero.name}立绘" decoding="async">
      </div>
      <div class="hero-choice-info">
        ...
        ${hero.quest ? `<p class="hero-choice-quest">${hero.quest}</p>` : ""}
        ${(() => { // V0.9.20 本命蛊：选人卡即养成墙——形态名 + 道行进度
          const bg = BENMING_GU[id]; if (!bg) return "";
          const bi = getBenmingStageInfo(id);
          return `<p class="hero-benming-line ${bi.stage > 0 ? "is-awake" : ""}" title="${bg.name}·${bi.stageName}｜${bg.stagePassives[bi.stage]}${bi.next ? `｜距${bi.next.name}还差 ${bi.toNext} 道行` : ""}">
            <b>${bg.glyph}</b>${bg.name}·${bi.stageName}<i>道行 ${bi.dao}${bi.next ? ` / ${bi.next.threshold}` : " · 圆满"}</i></p>`;
        })()}
      </div>
    </button>`;
  }).join("");
