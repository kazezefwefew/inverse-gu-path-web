...
const GAME_VERSION = "V0.9.21 选人舞台";
...
// 战斗内被动判定：本局蛊修是该蛊之主，且形态已到档。
function benmingPassive(heroId, stage) {
  return !!runState && runState.heroId === heroId && getBenmingStage(heroId) >= stage;
}
...
function refreshModalLock() {
  const hasModal = [dom.resultOverlay, dom.deckOverlay, dom.tutorialOverlay, dom.prologueOverlay, dom.loreOverlay, dom.balanceOverlay, dom.runStatsOverlay, dom.trialSettingsOverlay, dom.settingsOverlay, dom.overwriteConfirmOverlay, dom.relicOfferOverlay] // V0.9.9.2：遗物抉择弹窗纳入模态锁；V0.9.18 序章弹窗纳入
