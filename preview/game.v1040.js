...
function renderGulu() {
  ...
  const slots = s.slots.map((slot, i) => {
    if (!slot) {
      ...
      return `<div class="gulu-slot is-empty"><h4>第 ${i + 1} 栏 · 空圃</h4><div class="gulu-grades">${btns}</div></div>`;
    }
    if (slot.state === "egg") {
      const grade = GULU_GRADES[slot.grade];
      return `<div class="gulu-slot is-egg"><h4>第 ${i + 1} 栏 · ${grade.name}蛊卵</h4>
        <p class="gulu-egg-glyph">卵</p><p class="gulu-remain">${formatGuluRemain(slot.hatchAt - now)}破壳</p></div>`;
    }
    const card = CARD_LIBRARY[slot.cardKey];
    return `<div class="gulu-slot is-gu grade-${slot.grade}"><h4>第 ${i + 1} 栏 · ${slot.name}</h4>
  ...
  dom.guluBody.innerHTML = `
    ...
    <section class="gulu-sec"><h3>材料仓 <small>塔中通关全额带出 · 陨落折四成 · 残核仅通关可带</small></h3><div class="gulu-mats">${matChips}</div></section>
    <section class="gulu-sec"><h3>蛊圃 <small>孵卵按真实时间破壳 · 成蛊可带入塔（${carriedCount}/${GULU_CARRY_MAX}）或喂给本命蛊</small></h3><div class="gulu-slots">${slots}</div></section>
    <section class="gulu-sec"><h3>本命蛊 · ${heroGu?.name || "—"}</h3>
      <p class="gulu-benming ${injured ? "is-injured" : ""}">${heroGu?.glyph || "蛊"} ${bi.stageName} · 道行 ${bi.dao}${bi.next ? ` / ${bi.next.threshold}` : " · 圆满"}
      ${injured ? `<b>静养中：${formatGuluRemain(s.injuryUntil - now)}复元（形态降一档）</b>` : ""}</p>
      <p class="gulu-tip">压制线：蛊卵≤凡品 · 幼虫≤灵品 · 成虫≤玄品 · 真形≤天品。越级喂养即蛊斗——胜则道行加倍，败则反噬静养。</p></section>
...
function createRunState() {
  ...
  // V0.9.22 蛊庐：携带蛊入塔（至多 2 只并入起手牌组；不消耗——只有喂给本命蛊才会失去）。
  getGuluStore().slots
...
function bindEvents() {
  ...
  dom.guluOverlay?.addEventListener("click", (event) => {
    ...
    if (feed && !feed.disabled) {
      playUiSfx();
      guluNoticeText = guluFeedToBenming(Number(feed.dataset.guluFeed)).text;
      renderGulu();
    }
  });
...
const DEV_ACTIONS = {
  ...
  "guluFast": () => { const s = getGuluStore(); const now = guluNow(); let n = 0; s.slots.forEach((slot) => { if (slot && slot.state === "egg") { slot.hatchAt = now; n++; } }); if (s.injuryUntil > now) { s.injuryUntil = now; n++; } saveGuluStore(); const news = settleGuluTime(); devNotify(`时间快进：${n} 项到期。${news.join(" ")}`, "important"); },
