...
const GAME_VERSION = "V0.9.24.1 离线护符";
...
function saveJsonStore(key, obj) {
  try { localStorage.setItem(key, JSON.stringify(obj)); } catch (e) { /* 存储不可用忽略，不影响游戏 */ }
}
...
function addBenmingDaoxing(heroId, amount) {
  ...
  try { localStorage.setItem(BENMING_KEY, JSON.stringify(store)); } catch (e) { /* 存储不可用则忽略 */ }
}
...
function saveGuluStore() { try { localStorage.setItem(GULU_KEY, JSON.stringify(getGuluStore())); } catch (e) { /* 存储不可用则忽略 */ } }
...
function guluFeedToBenming(slotIndex) {
  ...
  if (win) {
    ...
    try { localStorage.setItem(BENMING_KEY, JSON.stringify(bStore)); } catch (e) { /* 忽略 */ }
...
function refreshModalLock() {
  const hasModal = [dom.resultOverlay, dom.deckOverlay, dom.tutorialOverlay, dom.prologueOverlay, dom.loreOverlay, dom.balanceOverlay, dom.runStatsOverlay, dom.trialSettingsOverlay, dom.settingsOverlay, dom.overwriteConfirmOverlay, dom.relicOfferOverlay, dom.guluOverlay] // V0.9.9.2：遗物抉择弹窗纳入模态锁；V0.9.18 序章；V0.9.22 蛊庐
...
function showStartScreen() {
  ...
  if (dom.guluOverlay) { window.clearInterval(guluRefreshTimer); dom.guluOverlay.classList.add("hidden"); } // V0.9.22 蛊庐同收
...
function clearRunAutosave() {
  try { localStorage.removeItem(RUN_AUTOSAVE_KEY); } catch (err) { /* 忽略 */ }
}
...
function openShopRemovePicker() {
  const state = getShopState();
  if (state.remove || runState.deckCards.length <= 6 || runState.guStones < getShopPrice(18 + ((runState.layer3?.active ? 3 : runState.layer2?.active ? 2 : 1) - 1) * 6)) return;
...
function bindEvents() {
  ...
  dom.guluOverlay?.addEventListener("click", (event) => {
    ...
    if (hatch && !hatch.disabled) {
      ...
      guluNoticeText = guluStartHatch(Number(idx), gid).text;
      renderGulu();
      return;
    }
