...
const HEROES = {
  ...
  blood: {
    name: "绛妄", role: "血道女蛊修", glyph: "血", maxHp: 56, energy: 3, lifespan: 20,
  ...
  longevity: {
    name: "朝暮", role: "寿道焚命蛊修", glyph: "寿", maxHp: 52, energy: 3, lifespan: 42,
...
function getSatchelCap() {
  return (runState?.mode === "tian" && (runState.tianTier || 0) >= 3) ? 2 : SATCHEL_CAP;
}
...
const GAME_VERSION = "V0.9.26 蛊庐音景";
...
const MODE_TUNING = {
  normal: { hpMul: 1, atkMul: 1, rewardMul: 1, rareBoost: 0, bossHpMul: 1, bossAtkMul: 1 },
  elite: { hpMul: 1.25, atkMul: 1.15, rewardMul: 1.3, rareBoost: 0.15, bossHpMul: 1.35, bossAtkMul: 1.2 },
  // V0.9.9 子批6：死劫——九死无生。数值高但「平」（可预判=公平，不做随层指数尖刺）；配合无续局(permadeath)即「失误即死」。
  deathtrial: { hpMul: 1.5, atkMul: 1.4, rewardMul: 1.7, rareBoost: 0.35, bossHpMul: 1.7, bossAtkMul: 1.5 },
};
...
const BENMING_GU = Object.freeze({
  fate: {
    ...
    stagePassives: ["尚在卵中沉睡。", "开局命势 +1。", "每场首次命势圆满时，额外抽 1 张牌。", "命势圆满时，额外获得 1 点防御与 1 蛊石。"],
  },
  blood: {
    name: "赤茧蛊", glyph: "茧",
    lore: "血债结成的茧，茧里裹着没讨回来的旧仇。",
    stagePassives: ["尚在卵中沉睡。", "每场战斗开局自带 2 层血煞。", "血煞上限 +2。", "每场首次血煞满溢时，恢复 4 点生命。"],
  },
  poison: {
    name: "蜕鳞蛊", glyph: "鳞",
    lore: "万毒噬身那夜蜕下的第一片鳞，还带着牙印。",
    stagePassives: ["尚在卵中沉睡。", "每场首次施毒额外 +1 层。", "攻击中毒的敌人时，伤害 +2。", "每场开局敌人自带 2 层毒性。"],
  },
  longevity: {
    name: "灯芯蛊", glyph: "芯",
    lore: "半盏寿灯里没烧完的芯，一头连着朝，一头连着暮。",
    stagePassives: ["尚在卵中沉睡。", "寿元上限 +2。", "每局首次焚寿时，返还 1 点寿元。", "焚寿燃命的伤害加成 +25%。"],
  },
});
...
function getBenmingImagePath(heroId, stage) { return `assets/codex/benming/${heroId}-${stage}.jpg`; }
...
function getGuluStore() {
  ...
  while (s.slots.length < GULU_SLOTS) s.slots.push(null);
...
function renderGulu() {
  ...
  const slots = s.slots.map((slot, i) => {
    if (!slot) {
      const btns = Object.entries(GULU_GRADES).map(([gid, g]) => {
        ...
        return `<button type="button" class="gulu-grade grade-${gid}" data-gulu-hatch="${i}:${gid}" ${can ? "" : "disabled"}>${g.name}<small>${g.mats}材${g.core ? "+残核" : ""} · ${g.timeText}</small></button>`;
      }).join("");
    ...
    if (slot.state === "egg") {
      ...
      return `<div class="gulu-slot is-egg grade-${slot.grade}"><h4>第 ${i + 1} 圃 · ${grade.name}蛊卵</h4>
        <p class="gulu-egg-glyph">${GULU_GRADE_GLYPHS[slot.grade] || "卵"}</p><p class="gulu-remain">${formatGuluRemain(slot.hatchAt - now)}破壳</p></div>`;
    }
    const card = CARD_LIBRARY[slot.cardKey];
    const art = getGuluCardArt(slot.cardKey);
    return `<div class="gulu-slot is-gu grade-${slot.grade}"><h4>第 ${i + 1} 圃 · ${slot.name}</h4>
      <div class="gulu-gu-art"><i>${GULU_GRADE_GLYPHS[slot.grade] || "蛊"}</i>${art ? `<img src="${art}" alt="${slot.name}" loading="lazy" decoding="async" onerror="this.remove()">` : ""}
  ...
  const events = s.events.slice(-8).reverse().map((e) => `<li>${e.text}</li>`).join("") || "<li>蛊庐尚静，塔中带出材料便可孵卵。</li>";
  ...
  const altarStages = BENMING_STAGES.map((st) => `<li class="${bi.stage >= st.stage ? "is-reached" : ""} ${bi.stage === st.stage ? "is-current" : ""}">
    <b>${st.name}</b><span>${heroGu?.stagePassives?.[st.stage] || ""}</span></li>`).join("");
  const altar = heroGu ? `
    <aside class="gulu-altar ${injured ? "is-injured" : ""}">
      <h3>本命蛊祭坛</h3>
      <div class="gulu-altar-art"><i>${heroGu.glyph}</i><img src="${getBenmingImagePath(heroId, bi.stage)}" alt="${heroGu.name}" loading="lazy" decoding="async" onerror="this.remove()"></div>
  ...
  dom.guluBody.innerHTML = `
    ${guluNoticeText ? `<p class="gulu-notice">${guluNoticeText}</p>` : ""}
  ...
  // V0.9.23 破壳强反馈：本次结算里有新破壳且蛊庐可见 → 最新一只走破壳仪式（按品阶分色）
  const hatched = news.filter((t) => t.includes("破卵"));
  if (hatched.length && dom.guluOverlay && !dom.guluOverlay.classList.contains("hidden")) {
    const newest = s.slots.filter((g) => g && g.state === "gu").sort((a, b) => (b.hatchAt || 0) - (a.hatchAt || 0))[0];
    if (newest) {
...
function guluAudioTick() {
  ...
  // 夜间虫鸣：真实时间昼夜，跨界时 2s 淡变
  const night = isGuluNight();
  if (night !== guluNightOn) {
    guluNightOn = night;
    if (night) window.AudioManager?.playAmbient?.("guluNight", { fadeMs: 2000 });
    else window.AudioManager?.stopAmbient?.({ fadeMs: 2000 });
  }
}
...
function getBloodMax() {
  const benmingBonus = benmingPassive("blood", 2) ? 2 : 0; // V0.9.20 赤茧蛊·成虫：血煞上限 +2
...
function cacheDom() {
  [
    "startScreen", "mobileOrientationOverlay", "mobileLogButton", "mobileAudioToggle", "mobileAudioClose", "heroChoices", "relicChoices", "modeChoices", "advancedCardPreview", "startBattleButton", "runProgress", "trialModeHint",
...
function grantBattleItem(itemId, sourceName = "命途所得") {
  ...
  if (runState.satchel.length >= getSatchelCap()) {
    gainGuStones(4, `丹囊已满，「${item.name}」折算`, { raw: true });
    return false;
  }
...
function createMapState({ seed = "", mode = trialMode, random = null } = {}) {
  ...
  /* 段6（step BOSS_ROUTE_STEP=6）：Boss */
  const bossSegment = [{
    id: "boss-1",
...
function createRunState() {
  ...
  const maxHp = Math.round((hero.maxHp + (relicId === "jadeMarrow" ? 8 : 0)) * (tianTier >= 9 ? 0.9 : 1)); // 九重·残躯：生命上限 -10%
  // V0.9.9 寿道·子批2c：maxLifespan=初始寿元；饲岁轮 +12 且起始寿元同涨（满寿入场）。
  // V0.9.20 灯芯蛊·幼虫：寿道本命蛊寿元上限 +2（同涨起始寿元）。
  const benmingLifespanBonus = (progression.selectedHeroId === "longevity" && getBenmingStage("longevity") >= 1) ? 2 : 0;
  ...
  const carriedGuIds = [];
  getGuluStore().slots
    .filter((g) => g && g.state === "gu" && g.carry && CARD_LIBRARY[g.cardKey])
...
function createBattleState() {
  ...
  const __mt = getModeTuning(); // V0.9.19：走统一入口（十重天按重数计算；此前直查 MODE_TUNING 会让 tian 回落普通档）
  ...
  dom.heroChoices.innerHTML = `
    ...
    <div class="hero-stage tone-${stageHeroId}" data-hero-id="${stageHeroId}">
      ...
      <div class="hero-stage-info">
        ...
        ${stageGu ? `<p class="hero-benming-line ${stageBi.stage > 0 ? "is-awake" : ""}" title="${stageGu.name}·${stageBi.stageName}｜${stageGu.stagePassives[stageBi.stage]}${stageBi.next ? `｜距${stageBi.next.name}还差 ${stageBi.toNext} 道行` : ""}">
...
function getMapTransitionText(type) {
  switch (type) {
    ...
    case "boss": return runState?.layer3?.active
      ? "绝域之主盘踞，杀机已现"
      : (runState?.layer2?.active ? "生态之主盘踞，杀机已现" : "尸盘转动，守关者苏醒");
    default: return "命途流转";
  }
}
...
function focusCurrentMapSegment() {
  ...
  const currentStep = Math.max(1, Math.min(maxStep, Number(runState.currentRouteStep) || 1));
...
function renderMapScreen() {
  ...
  // V0.9.12.1 修复：进三层后 layer2.active 仍为 true（历史约定），此前三层地图全程串台二层"生态"文案——layer3 优先分流。
  const __isL3Map = runState.layer === 3 || runState.layer3?.active;
  const __isL2Map = !__isL3Map && (runState.layer === 2 || runState.layer2?.active);
  ...
  dom.mapRoute.innerHTML = runState.mapState.segments.map((segment, index) => {
    ...
    const layerName = __isL3Map
      ? (runState.layer3?.routeName || "绝域深径")
      : (runState.layer2?.routeName || "生态深径");
...
function enterMapNode(node) {
  ...
  if (node.type === "reward" && runState.layer3?.active) {
  ...
  lockSiblingNodes(node);
  addLog(`命途分岔：你选择了${node.name}。`, "important");
  if (node.enemyId && runState.layer3?.active && typeof layer2MarkBestiary === "function") layer2MarkBestiary(node.enemyId);
...
/* ===================== V0.9.8.7 自动续局存档 =====================
 * 唯一真相 = runState（不引入第三套状态）。仅在「地图态」（节点之间、currentNode 为空）写档，
 * 不序列化战斗态；战斗中途退出 → 续局回地图、当前节点重打。一局结束（通关/死亡）清档。 */
const RUN_AUTOSAVE_KEY = "nmg.run.autosave";
const RUN_AUTOSAVE_VERSION = 1;

function saveRunStateToStorage() {
  try {
    // 仅在地图态写档：本局进行中 + currentNode 为空（战斗/事件/蛊坊/休整中途不覆盖）。
    if (!runState || runState.status !== "running" || runState.currentNode) return;
    ...
    if (dom.resultOverlay && !dom.resultOverlay.classList.contains("hidden")) return;
...
function parseRunAutosaveRaw(raw) {
  ...
  try {
    ...
    // 完整性校验：进行中、英雄合法、地图段合法。任一不符视为坏档。
    if (run.status !== "running" || !run.heroId || !HEROES[run.heroId]) return null;
    if (!run.mapState || !Array.isArray(run.mapState.segments) || !run.mapState.segments.length) return null;
    return payload;
...
function resumeRunFromAutosave() {
  ...
  game = null;
  runState.currentNode = null; // 续局一律落在地图态（保险）
  resetAllLogs();
  const layerName = runState.layer === 3 ? "第三层" : runState.layer === 2 ? "第二层" : "第一层";
  addJourneyLog(`续局：自命途种子 ${runState.trialSeed || "未知"} 处接续，停于${layerName}第 ${runState.currentRouteStep} 段。`, "important");
  showMapScreen();
  return true;
}
...
function updateResumeRunButton() {
  ...
  const layerName = run.layer === 3 ? "第三层" : run.layer === 2 ? "第二层" : "第一层";
  if (dom.resumeRunSummary) dom.resumeRunSummary.textContent = `${hero ? hero.name : "蛊修"} · ${layerName}第 ${run.currentRouteStep} 段 · 生命 ${run.currentHp}/${run.maxHp}`;
...
function showNewRunOverwriteConfirm() {
  ...
  if (dom.overwriteConfirmText && payload) {
    ...
    dom.overwriteConfirmText.textContent = `开新局将覆盖上一局存档（${hero ? hero.name : "蛊修"} · 第 ${run.currentRouteStep} 段），确定继续？`;
  }
...
function createLayer2MapState(theme) {
  ...
  /* 段6（step BOSS_ROUTE_STEP=6）：第二层 Boss */
  const seg6 = [
    { id: "l2-6-boss", step: BOSS_ROUTE_STEP, type: "boss", enemyId: pool.boss, layer2: true, l2theme: theme,
 ...
 * 复用同一地图渲染/点击/推进管线（runState.layer=3、mapState 替换）。
...
function getLayer3ThemeText(kind) {
  const theme = runState?.layer3?.theme;
  const isBone = theme === "bone";
  const routeName = runState?.layer3?.routeName || (isBone ? "骨塔高陵" : "蜂窟魔巢");
  const map = {
    rest: {
...
function createLayer3MapState(theme) {
  ...
  /* 段6（step BOSS_ROUTE_STEP=6）：第三层 Boss */
  const seg6 = [
    { id: "l3-6-boss", step: BOSS_ROUTE_STEP, type: "boss", enemyId: pool.boss, layer3: true, l3theme: theme,
...
function layer3CompleteNodeAndReturnMap() {
  ...
  if (node.type === "boss") {
    ...
    layer3MarkProgress(st.routeId === "bone" ? "boneBossDefeated" : "beehiveBossDefeated");
    runState.currentNode = null;
    showLayer3Conclusion(true);
    return;
  }
...
function showCommandPathChoice() {
  ...
  dom.resultDescription.textContent = "深泽已破，蛊息却未止。远处骨塔有铃声摇动，蜂窟中万翅齐鸣…就此结算收功，抑或继续深入更深的绝域？";
  dom.resultDeckButton?.classList.remove("hidden");
  dom.resultStatsButton?.classList.remove("hidden");
  dom.resultLoreButton?.classList.remove("hidden");
  dom.resultFeedbackButton?.classList.add("hidden");
...
/* 选择「就此结算」：走二层结算流程（镜像 settleAtLayer1） */
...
function showLayer3Branch(node) {
  const st = runState.layer3;
  const route = LAYER3_ROUTES[st.routeId];
...
function chooseLayer3Branch(kind) {
  ...
  dom.runSummary.classList.add("hidden");
  runState.currentNode = {
    id: `layer3-${st.routeId}-${st.nodesCleared || 0}`,
    step: MAX_ROUTE_STEP, type: kind,
    icon: kind === "event" ? "缘" : (kind === "rest" ? "息" : "坊"),
    name: kind === "event" ? "绝域机缘" : (kind === "rest" ? "塔隙休整" : "残灯蛊坊"),
    description: MAP_NODE_DESCRIPTIONS[kind] || "", layer3: true,
  };
  dom.resultOverlay.classList.add("hidden");
  refreshModalLock();
...
function openLayer3Reward(node) {
  const st = runState.layer3 || {};
  const route = LAYER3_ROUTES[st.routeId] || { name: st.routeName || "绝域深径", loreId: "unfinished", favoredCardKeys: [] };
...
/* 第三层结算页（镜像 showLayer2Conclusion，追加第三层统计块） */
function showLayer3Conclusion(cleared) {
  const st = runState.layer3 || {};
  runState.status = "cleared";
  showRunConclusion(true);
  const route = LAYER3_ROUTES[st.routeId] || null;
...
function currentLayerKey() { return runState?.layer3?.active ? 3 : (runState?.layer2?.active ? 2 : 1); }
...
function openChanceEvent() {
  ...
  dom.resultOverlay.querySelector(".result-card").className = "result-card map-result";
  hideRewardPanels();
  const __l2Event = runState.layer3?.active ? getLayer3ThemeText("event") : (runState.layer2?.active ? getLayer2ThemeText("event") : null);
...
function resolveRestChoice(choice) {
  ...
  if (choice === "heal") {
    ...
    const healed = healRunHp(12 + restBonus, runState.currentNode.name);
...
function openShopNode() {
  ...
  hideRewardPanels();
  const __l2Shop = runState.layer3?.active ? getLayer3ThemeText("shop") : (runState.layer2?.active ? getLayer2ThemeText("shop") : null);
...
function renderShop() {
  ...
  const removePrice = getShopPrice(18 + ((runState.layer3?.active ? 3 : runState.layer2?.active ? 2 : 1) - 1) * 6); // V0.9.8.5：删牌价随层递增 18/24/30
...
function showRemovePickerOverlay({ eyebrow, title } = {}) {
...
function openShopRemovePicker() {
  const state = getShopState();
  const price = getShopPrice(18 + ((runState.layer3?.active ? 3 : runState.layer2?.active ? 2 : 1) - 1) * 6);
...
function startFloorBattle() {
  ...
  // V0.9.8.5 修：必须先判第三层——进三层后 runState.layer2.active 仍为 true（enterLayer3Map 未清），若先判 layer2 会在三层错放二层曲（瘴林/血沼）。与节点完成/finishBattle 的"layer3 优先"约定一致。
  if (runState.layer3?.active) {
    musicScene = runState.layer3.routeId === "beehive" ? "layer3Beehive" : "layer3Bone";
  } else if (runState.layer2?.active) {
    musicScene = runState.layer2.routeId === "bloodmarsh" ? "layer2Bloodmarsh" : "layer2Miasma";
  } else {
  ...
  runState.furnaceResolved = runState.currentNode?.type !== "elite" && runState.currentNode?.type !== "defy"; // V0.9.8.6 逆命也开炉
  switchLogChannel("battle");
  resetBattleLog();
  // V0.9.18 塔中回声：Boss 战前对峙文本（按敌人 id）——进战斗日志，并作为开场压迫感横幅（见下方 setBattleMessage，覆盖通用 intro）。
  const bossTaunt = isBossNode ? (BOSS_TAUNTS[game.enemy?.id] || "") : "";
  ...
  if (benmingPassive("blood", 1)) {
    gainBlood(2);
    addLog("赤茧蛊苏醒：开局血煞 +2。", "positive-log");
  }
  if (benmingPassive("fate", 1)) {
    game.player.fateMomentum = Math.min(FATE_MOMENTUM_MAX, (game.player.fateMomentum || 0) + 1);
    addLog("衔命虫牵线：开局命势 +1。", "positive-log");
  }
  if (benmingPassive("poison", 3) && game.enemy) {
    applyEnemyPoison(2, "蜕鳞蛊", { corrosive: false });
    addLog("蜕鳞蛊先行探路：敌人开场中毒 2 层。", "positive-log");
  }
...
function getCurrentEnemyAction() {
  ...
  if (game.enemy.phase2) {
    ...
    if (game.enemy.id === "bloodRobeMotherBoss") {
      ...
      if (game.enemy.intent === "crimsonGather") return { ...action, bonus: 9, lifesteal: 6 };
    }
    // 第三层 Boss · 骨巢守墓王「骨巢开裂」：重击与蓄力增强
    if (game.enemy.id === "boneNestGuardianBoss") {
      if (game.enemy.intent === "tombCrush") return { ...action, damage: 14 };
      if (game.enemy.intent === "boneVolley") return { ...action, damage: 8, hits: 2 };
      if (game.enemy.intent === "sepulchreCharge") return { ...action, bonus: 12, interruptThreshold: 16 };
    }
    // 第三层 Boss · 灾厄蜂后「蜂群暴动」：多段与毒刺增强
    if (game.enemy.id === "calamityQueenBoss") {
      if (game.enemy.intent === "queenSting") return { ...action, damage: 6, hits: 3, playerPoisonSting: 2 };
      if (game.enemy.intent === "swarmBurst") return { ...action, damage: 7, playerPoison: 4 };
      if (game.enemy.intent === "broodCharge") return { ...action, bonus: 10 };
    }
  }
  return action;
}
...
function renderHeroDetail(it) {
  ...
  return '<button type="button" class="wangulu-back" data-gu-back>‹ 返回蛊录</button>'
    ...
    + '<section class="wangulu-sec"><h4>立绘</h4><div class="wangulu-art">' + artHtml + '<i class="wangulu-art-stage">' + escGu(it.stage || "") + '</i></div></section>'
    + '<section class="wangulu-sec"><h4>蛊修资料</h4>' + row("道脉", it.faction) + row("形态", it.type) + row("初始生命", it.heroHp) + row("初始真元", it.heroEnergy) + row("初始寿元", it.heroLifespan) + '</section>'
    + renderHeroSealSection(it)
    + '<section class="wangulu-sec"><h4>本命被动</h4><p class="wangulu-effect">' + escGu(it.gameplayEffect || "") + '</p><p class="wangulu-short">' + escGu(it.descriptionShort || "") + '</p></section>'
    + '<section class="wangulu-sec"><h4>列传</h4><p class="wangulu-lore">' + escGu(it.descriptionLore || "") + '</p>' + row("出处", it.dropsFrom) + row("录入", it.unlockCondition) + '</section>'
    + '</article>';
}
...
function renderHeroSealSection(it) {
  ...
  const questHtml = entry[1].quest
    ? '<section class="wangulu-sec"><h4>所求</h4><p class="wangulu-lore">' + escGu(entry[1].quest) + '</p></section>'
    : "";
...
function renderRelicDetail(it) {
  ...
  return '<button type="button" class="wangulu-back" data-gu-back>‹ 返回蛊录</button>'
    ...
    + '<section class="wangulu-sec"><h4>' + (it.image ? '立绘' : '纹样') + '</h4><div class="wangulu-art">' + artHtml + '</div></section>'
    + '<section class="wangulu-sec"><h4>遗物功用</h4><p class="wangulu-effect">' + escGu(it.gameplayEffect || "") + '</p></section>'
    + '<section class="wangulu-sec"><h4>录入</h4>' + row("类别", it.kind) + row("道脉", it.faction) + row("获得", it.dropsFrom) + '</section>'
    + '</article>';
}
...
function resolveCard(card) {
  ...
  switch (card.key) {
    ...
    case "burningEssence":
      ...
      addLog(`你使用燃元蛊，失去 ${v.selfDamage} 点生命，获得 ${v.energy} 点真元${v.draw > 0 ? `，抽 ${v.draw} 张牌` : ""}。`, "damage-log");
...
function applyFateCardFlow(card) {
  ...
  // V0.9.9.2 连势符：连续打出同类型蛊也累积命势（不再要求交替）
  if (lastFlow && (lastFlow !== currentFlow || hasOrdinaryRelic("chainFate"))) {
    gainFateMomentum(1);
  }
...
function gainFateMomentum(amount) {
  ...
  // V0.9.8.5：满层每回合最多爆发 2 次（斩断回合内无限雪球）；封顶后命势保留为满层带入下回合，不浪费、不碰"交替出牌得命势"特性。
  if (game.player.fateMomentum >= FATE_MOMENTUM_MAX && (game.fateBurstsThisTurn || 0) < 2) {
    ...
    if (hasOrdinaryRelic("fateSurge")) { // 势盈引：圆满额外抽 1
      drawCards(1);
      addLog("势盈引：命势圆满，额外抽 1 张牌。", "positive-log");
      notifyRelicTrigger("fateSurge", "圆满·额外抽1");
    }
    if (hasOrdinaryRelic("fateBurst") && game.enemy && game.enemy.hp > 0) { // 势爆符：圆满直伤 6（无视护甲）
      const __fb = 6;
      game.enemy.hp = Math.max(0, game.enemy.hp - __fb);
      recordPlayerDamage(__fb, { card: true });
      spawnDelayedFloatText(dom.enemyPortrait, `命势爆 -${__fb}`, "fate-float", 60);
      addLog(`势爆符：命势圆满，直击敌人 ${__fb} 点（无视护甲）。`, "player-log");
      notifyRelicTrigger("fateBurst", `圆满·直伤${__fb}`);
      // V0.9.12.1 修复：势爆符直伤跨半血此前不触发任何 Boss 转阶段检查
      checkCorpseDiskPhase2();
      checkLayer2BossPhase2();
    }
    // V0.9.20 衔命虫·成虫/真形：首次圆满额外抽1；圆满余泽防御+1蛊石+1
    ...
    if (benmingPassive("fate", 3)) {
      game.player.armor += 1;
      recordArmorGained(1);
      gainGuStones(1, "衔命虫", { raw: true });
      addLog("衔命虫衔来余泽：防御 +1，蛊石 +1。", "positive-log");
    }
...
function getLifespanDamageBonus(player) {
  ...
  if (benmingPassive("longevity", 3)) bonus = Math.round(bonus * 1.25); // V0.9.20 灯芯蛊·真形：焚寿加成 +25%
  return bonus;
}
...
function resolveAttack(card, baseDamage, detail = "") {
  ...
  const benmingVenomBonus = (benmingPassive("poison", 2) && (game.enemy?.poison || 0) > 0) ? 2 : 0;
  const modifiedBase = baseDamage + game.attackBonus + bloodBonus + instanceBonus + boneVenomBonus + benmingVenomBonus;
...
function resolveEnemyTurn() {
  ...
  if (action.kind === "charge") {
    ...
    let rawDamage = Math.round(action.damage * hitCount * (game.enemyAttackMultiplier || 1)) + game.enemy.chargedBonus + lowHpBonus + enrageBonus + routeBonus;
    ...
    const weakenCut = Math.min(rawDamage, game.enemy.weaken || 0);
    if (weakenCut > 0) rawDamage -= weakenCut;
...
function finishBattle(victory) {
  ...
  if (victory) {
    ...
    if (runState.currentNode?.type === "boss") {
    ...
    if (hasOrdinaryRelic("stoneInterest")) gainGuStones(3, "蛊石生息"); // V0.9.9.2 蛊石生息：每段战斗胜利额外 3 蛊石
  ...
  if (victory) {
    ...
    if (runState.currentNode?.type === "elite" || runState.currentNode?.type === "defy") {
    ...
    if (runState.layer3?.active) {
      const __l3Node = runState.currentNode?.name || "未知节点";
      const __l3Enemy = game.enemy?.definition?.name || "未知敌人";
      getRunStats().deathNode = `第三层 · ${runState.layer3.routeName || "绝域深径"} · ${__l3Node}`;
...
function openCardReward() {
  ...
  if (runState.currentNode?.type === "elite" || runState.currentNode?.type === "defy") {
    const __defy = runState.currentNode?.type === "defy"; // V0.9.8.6
...
function getFurnaceProbabilities(entry, materialId) {
  ...
  if (isMaterialMatched(entry, materialId)) return { stable: 75, mutation: 20, backlash: 5, label: "材料匹配" };
...
function renderFurnaceRouteSummary(entry, plan) {
  ...
  const backlashText = "反噬可能：蛊损暂歇、反伤 6 生命、折损 2 寿元，或偏斜 +1 但附带小代价（不再永久卡损或砍最大生命）。";
...
function resolveFurnacePlan(entry, plan) {
  ...
  const tianFurnaceShift = (runState?.mode === "tian" && (runState.tianTier || 0) >= 6) ? 10 : 0;
  if (roll < plan.probabilities.stable - tianFurnaceShift) return applyStableFurnace(entry, plan.materialId);
  if (roll < plan.probabilities.stable - tianFurnaceShift + plan.probabilities.mutation) {
...
function advanceToNextFloor() {
  ...
  if ((runState.currentNode?.type === "elite" || runState.currentNode?.type === "defy") && !runState.furnaceResolved) return; // V0.9.8.6 逆命也须等炼炉结算
  dom.resultOverlay.classList.add("hidden");
  refreshModalLock();
  if (runState?.layer3?.active) { layer3CompleteNodeAndReturnMap(); return; }
  if (runState?.layer2?.active) { layer2CompleteNodeAndReturnMap(); return; }
...
function snapshotDeathContext() {
  ...
  return {
    ...
    layer: (rs.layer3 && rs.layer3.active) ? 3 : ((rs.layer2 && rs.layer2.active) ? 2 : 1),
    route: (rs.layer3 && rs.layer3.routeName) ? rs.layer3.routeName : ((rs.layer2 && rs.layer2.routeName) ? rs.layer2.routeName : ""),
...
function generateRunTitle(stats, runState, cleared) {
  ...
  const l3 = rs.layer3 || {};
  const l3Route = String(l3.routeName || "");
  const l3Theme = String(l3.theme || "");
  const l3BossDefeated = !!(s.layer3BossDefeated || l3.bossDefeated);
  const l3Entered = !!(s.layer3Entered || l3.active);
  const deathNode = String(s.deathNode || "");
  const inLayer3Death = !cleared && (l3Entered || deathNode.indexOf("第三层") >= 0);
  ...
  if (l3BossDefeated) return wrap("layer3Clear");
  // ⓪ 败于三层 Boss 留名
  if (deathEnemy.indexOf("骨巢守墓王") >= 0) return wrap("boneBossFall");
  if (deathEnemy.indexOf("灾厄蜂后") >= 0) return wrap("queenFall");
  // ⓪ 三层路线阵亡
  if (inLayer3Death && isBoneRoute) return wrap("boneRouteFall");
  if (inLayer3Death && isBeehiveRoute) return wrap("beehiveRouteFall");
  // ① 通关二层
  if (l2BossDefeated) return wrap("layer2Clear");
  // ② 败于二层 Boss 留名
  if (deathEnemy.indexOf("百瘴母蛊") >= 0) return wrap("miasmaBossFall");
  if (deathEnemy.indexOf("血衣蛊母") >= 0) return wrap("bloodBossFall");
  // ③ 击败一层 Boss 未进二层（通关收束）
  if (cleared && !l2Entered) return wrap("layer1Clear");
  // ④ 死亡地点（瘴/血）——置于「深泽初探」之前，确保专属称号可达
  if (!cleared && route.indexOf("瘴") >= 0) return wrap("miasmaFall");
  if (!cleared && (route.indexOf("血沼") >= 0 || route.indexOf("血") >= 0)) return wrap("bloodmarshFall");
  // ⑤ 进二层未破二层 Boss 而死（无明确路线染色时的兜底）
  if (l2Entered && !l2BossDefeated && !cleared) return wrap("layer2Unfinished");
  // ⑥ 进二层但 stats 标记入二层、route 缺失的更弱兜底
  if (l2Entered && !cleared) return wrap("layer2Explore");
  // ⑦ 流派
  ...
  if (!cleared && routeBand === "early") return wrap("earlyFall");
  if (!cleared && routeBand === "middle") return wrap("midFall");
  if (!cleared && routeBand === "late") return wrap("lateFall");
  // 兜底
...
function showRunConclusion(cleared) {
  clearRunAutosave(); // V0.9.8.7 自动续局：一局结束（通关/死亡）即清档，开始界面不再显示「继续上一局」
  if (!cleared) bumpSimingDeaths(); // V0.9.18 塔中回声：陨落一次，司命人跨局记住
  // V0.9.8.3：首次通关任意路线即解锁精英模式（覆盖一/二/三层全部 cleared 汇聚点）。
  if (cleared && !progression.eliteUnlocked) {
  ...
  // V0.9.9 子批6：精英通关→解锁死劫；死劫通关→得金印（持久成就）+ 结算金身样式。
  if (cleared && runState.mode === "elite" && !progression.deathtrialUnlocked) {
  ...
  // V0.9.19 十重天：通关落天梯进度（按英雄取最高，通第 N 重解锁第 N+1 重）。
  if (cleared && runState.mode === "tian") {
  ...
  // V0.9.14 蛊修印录：通关落英雄×模式印记（每英雄独立，见万蛊录「蛊修印录」页与选人卡角标）。
  if (cleared) markHeroSeal(runState.heroId, runState.mode || "normal");
  // V0.9.20 本命蛊：局末结道行（输赢都有：每胜一战×2 + 通关15，×模式奖励系数），跨形态阈值则弹进化仪式。
  {
    const benmingEarn = Math.round((runState.defeatedEnemies.length * 2 + (cleared ? 15 : 0)) * (getModeTuning().rewardMul || 1));
    if (benmingEarn > 0) {
      const gu = BENMING_GU[runState.heroId];
  ...
  // V0.9.22 蛊庐：材料带出——通关全额、陨落折四成；蛊母残核仅通关带出（阵亡散逸）。
  {
    const guluStore = getGuluStore();
    const ratio = cleared ? 1 : 0.4;
    const takenParts = [];
    MATERIAL_IDS.forEach((id) => {
      const have = Number(runState.materials?.[id]) || 0;
      const take = Math.floor(have * ratio);
      if (take > 0) {
        guluStore.materials[id] = (guluStore.materials[id] | 0) + take;
        takenParts.push(`${MATERIALS[id].name}×${take}`);
      }
    });
    const cores = cleared ? (runState.bossCores | 0) : 0;
    if (cores > 0) { guluStore.bossCores = (guluStore.bossCores | 0) + cores; takenParts.push(`蛊母残核×${cores}`); }
    if (takenParts.length) {
      guluPushEvent(guluStore, `塔中带出：${takenParts.join("、")}${cleared ? "" : "（陨落仅四成入库）"}。`);
      saveGuluStore();
      addLogToChannel("journey", `蛊庐入库：${takenParts.join("、")}${cleared ? "" : "（陨落仅四成入库）"}。`, "system-log");
    }
    if (!cleared && (runState.bossCores | 0) > 0) {
  ...
  dom.eventPanel?.classList.add("hidden");
  dom.shopPanel?.classList.add("hidden");
  dom.resultSeal.textContent = cleared ? (deathtrialJustCleared ? "劫" : "通") : "绝";
  dom.resultEyebrow.textContent = cleared ? (deathtrialJustCleared ? "死劫 · 九死无生" : "命途分岔 · 功成") : `止步第 ${runState.currentRouteStep} 段`;
  dom.resultTitle.textContent = cleared ? (deathtrialJustCleared ? "死劫·渡尽" : "命途塔通关") : "道途断绝";
  ...
  if (cleared) {
    ...
    dom.resultDescription.textContent = getRunEvaluation(true) + (HERO_ENDINGS[runState.heroId] ? "\n\n" + HERO_ENDINGS[runState.heroId] : "");
  } else {
    dom.resultDescription.style.whiteSpace = "";
    dom.resultDescription.textContent = `${stats.deathEnemy || game.enemy?.definition?.name || "未知敌人"}终结了此局命途，死于${deathStepText}。${getRunEvaluation(false)}`;
  }
  ...
  const __l3Active = !!__l3.active;
  const __routeName = String((__l3Active && __l3.routeName) || (runState.layer2 && runState.layer2.routeName) || (stats.deathContext && stats.deathContext.route) || "");
  ...
  const __layer = __l3Active ? 3 : ((runState.layer2 && runState.layer2.active) ? 2 : 1);
  const __layerText = __layer === 3
    ? `第三层 · ${__l3.routeName || "绝域深径"}`
    : (__layer === 2 ? `第二层 · ${(runState.layer2 && runState.layer2.routeName) || "生态深径"}` : "第一层 · 命途塔");
  const __routeLine = __layer === 3
    ? `${__l3.routeName || "绝域深径"} · Boss${__l3.bossDefeated ? "已破" : "未破"} · 终点「${__l3.lastNodeName || "-"}」`
    : (__layer === 2
  ...
  const __finalNode = cleared
    ? (__layer === 3 ? (__l3.lastNodeName || "绝域尽头") : (__layer === 2 ? ((runState.layer2 && runState.layer2.lastNodeName) || "生态尽头") : "塔顶尸盘"))
  ...
  dom.runSummary.innerHTML = `
    ...
    <section class="run-summary-section run-sec-result ${cleared ? "run-sec-clear" : "run-sec-death"}">
      ...
      <p class="run-sec-sub">入塔蛊修 ${hero?.name || "未知蛊修"} · 本命遗物 ${relic?.name || "未知遗物"} · ${getTrialModeInfo(runState.trialMode)?.name || "标准试炼"} · 本局模式 <strong>${getRunModeLabel(runState.mode)}</strong></p>
    </section>
...
function renderIntent() {
  ...
  if (action.kind === "charge") {
    ...
    const chargeArmorText = action.armor
      ...
      : "";
    dom.intentDescription.textContent = `本回合不攻击；下一次攻击 +${action.bonus}${chargeArmorText}${chargeExtras.length ? `（本回合${chargeExtras.join("，")}）` : ""}`;
...
function bindEvents() {
  ...
  dom.removePickerClose?.addEventListener("click", () => { playUiSfx(); cancelShopRemovePicker(); });
  dom.removePickerOverlay?.addEventListener("click", (event) => {
    if (event.target === dom.removePickerOverlay) cancelShopRemovePicker();
  });
  ...
  dom.shopCloseButton?.addEventListener("click", () => {
    playUiSfx();
    if (runState?.currentNode?.type === "shop") completeOverlayNode();
...
function devJumpBoss(opts) {
  ...
  if (opts.layer3) {
    runState.layer = 3;
    runState.layer3 = { active: true, routeId: opts.routeId, routeName: opts.routeName || "", theme: opts.routeId,
...
function devClearLayer3Test() {
  ...
  if (typeof runState !== "undefined" && runState) {
    runState.layer3 = null; if (runState.layer === 3) runState.layer = 2;
...
function devSimL3Death() {
  if (!devRequireRun()) return;
  runState.layer = 3;
  runState.layer3 = { active: true, routeId: "bone", routeName: "骨塔高陵", theme: "bone",
    branchChoice: "", bossDefeated: false, nodesCleared: 2, lastNodeName: "骨塔高陵·骨甲回廊" };
  const s = getRunStats();
  ...
  if (typeof showRunConclusion === "function") { showRunConclusion(false); devNotify("模拟：三层死亡结算（骨甲僵持死因）。", "important"); }
}
function devSimL3Clear() {
  if (!devRequireRun()) return;
  runState.layer = 3;
  runState.layer3 = { active: true, routeId: "beehive", routeName: "蜂窟魔巢", theme: "beehive",
    branchChoice: "", bossDefeated: true, nodesCleared: 4, lastNodeName: "蜂窟魔巢·之主" };
  const s = getRunStats();
  ...
  if (typeof showLayer3Conclusion === "function") { showLayer3Conclusion(true); devNotify("模拟：三层通关结算。", "important"); }
  else if (typeof showRunConclusion === "function") { showRunConclusion(true); }
}
...
const DEV_ACTIONS = {
  ...
  "eliteSimClear": () => { if (!devRequireRun()) return; runState.status = "cleared"; showRunConclusion(true); devNotify("模拟通关结算（触发精英解锁）。", "important"); },
...
const PRELOAD_ASSETS = [
  ...
  { url: "assets/audio/gulu/gulu-pot.v1.mp3", type: "audio", priority: 4, label: "陶罐开合" },
