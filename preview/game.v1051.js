...
const GAME_VERSION = "V0.9.32 防误触批2";
  ...
  dom.guluBody.innerHTML = `
    ...
    <div class="gulu-layout">
      <div class="gulu-main">
        ...
        <div class="gulu-lamp-row"><button type="button" class="gulu-lamp" data-xianghuo-open="gulu" aria-label="香火供奉"><i class="gulu-lamp-glyph" aria-hidden="true">灯</i><span>长明灯 · 添一炷香火</span></button></div>
      </div>
      ${altar}
    </div>`;
...
function getFurnaceTianShift() {
  return (runState?.mode === "tian" && (runState.tianTier || 0) >= 6) ? 10 : 0;
}
...
function resolveFurnacePlan(entry, plan) {
  ...
  if (backlashWidth > 0 && roll >= 100 - backlashWidth * 0.125) return applyReverseForge(entry, plan.materialId);
...
const FURNACE_RITE_STYLE = Object.freeze({
  stable:   { tone: "gold",  seal: "稳", eyebrow: "蛊炉 · 炉火稳定", sfx: "guluHatchLing", hint: "点击任意处 · 收蛊", text: "炉火稳稳收势，蛊纹更深一分。" },
  mutation: { tone: "tian",  seal: "异", eyebrow: "蛊炉 · 蛊性异变", sfx: "guluHatchXuan", hint: "点击任意处 · 收蛊", text: "材料扭转蛊性，一蛊化作它形。" },
  reverse:  { tone: "jade",  seal: "淬", eyebrow: "蛊炉 · 逆火淬体", sfx: "guluHatchTian", hint: "点击任意处 · 收蛊", text: "炉火本欲反噬，却被这蛊生生逆炼成器——大凶化大吉。" },
  backlash: { tone: "blood", seal: "噬", eyebrow: "蛊炉 · 炼蛊反噬", sfx: "hitHeavy",      hint: "点击任意处 · 忍痛", text: "炉火逆冲，蛊未成器，反噬其主。" },
});
function showFurnaceRite(outcome) {
  if (!outcome) return;
...
function confirmFurnaceUpgrade() {
  ...
  render();
  showNextFloorButton();
  showFurnaceRite(outcome); // V0.9.30 开炉全屏仪式（盖在结果页之上，点击/超时散场）
}
...
function showRunConclusion(cleared) {
  ...
  dom.runSummary.innerHTML = `
    ...
    ${((cleared || __layer >= 2) && !progression.xianghuoHidePrompt) ? `<section class="run-summary-section run-sec-xianghuo">
