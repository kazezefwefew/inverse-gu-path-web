...
const GAME_VERSION = "V0.9.33 手感与说明";
...
// V0.9.33 神化/归墟(stage4/5)暂无专属立绘→复用真形(stage3)图；日后补 heroId-4/5.jpg 时去掉 Math.min 即启用。
function getBenmingImagePath(heroId, stage) { return `assets/codex/benming/${heroId}-${Math.min(Number(stage) || 0, 3)}.jpg`; }
...
function enterLayer2Map(routeId) {
  ...
  // V0.9.33 BGM 冷加载治理：进二层地图即后台预热本路线 BGM（进战前有数秒足够下完 3~6MB），进战秒起播、不再卡顿。
...
function gainFateMomentum(amount) {
  ...
  if (game.player.fateMomentum >= FATE_MOMENTUM_MAX && (game.fateBurstsThisTurn || 0) < 2) {
    ...
    if (benmingPassive("fate", 3)) {
      const fateBoon = benmingPassive("fate", 5) ? 2 : 1; // 归墟：命势圆满余泽翻倍
