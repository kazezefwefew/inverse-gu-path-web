...
const TUTORIAL_PAGES = Object.freeze([
  {
    ...
    lines: [
      ...
      "衔命虫达到真形后，无名逆命者每局可在三相织命（改变命势生成）与噬签改命（改变圆满时机）中选择一条路线。",
      "绛妄：用生命换血煞，靠血道蛊打出爆发。",
      "青蟒：不断施毒，拖回合让毒性耗死敌人。",
  ...
  },
  {
    ...
    lines: [
      ...
      "三相织命要凑齐攻击、护甲、辅助；噬签改命盈满后会在敌人意图旁明确显示「改签」。",
      "血煞：血道资源，越高越利于爆发。",
...
function cacheDom() {
  [
    ...
    "intentBox", "intentIcon", "intentName", "intentDescription", "fateRewriteButton", "fateRewriteChoice", "enemyPower", "enemyStatusList", "arenaKicker",
  ...
  dom.benmingPathChoices.innerHTML = Object.values(BENMING_PATHS.fate).map((path) => `
    <button class="benming-path-choice ${selected === path.id ? "selected" : ""}" type="button"
      ...
      <span class="benming-path-copy">
        ...
        <em>${path.summary}</em>
        <i>${effectiveStage >= 5 ? `归墟：${path.guixu}` : `归墟后：${path.guixu}`}</i>
      </span>
    </button>`).join("");
}
...
function updateResumeRunButton() {
  ...
  if (dom.resumeRunSummary) dom.resumeRunSummary.textContent = `${hero ? hero.name : "蛊修"} · ${getBenmingPathDisplayName(run)} · ${layerName}第 ${run.currentRouteStep} 段 · 生命 ${run.currentHp}/${run.maxHp}`;
...
function completeFateRewrite({ useCandidate, fallback = false } = {}) {
  ...
  const oldName = getEnemyActionForIntent(oldIntent).name || "原意图";
  let newName = oldName;
  if (useCandidate && candidate?.intentId) {
    game.enemy.intent = candidate.intentId;
    newName = getCurrentEnemyAction().name || "新意图";
    getRunStats().fateRewrites = (getRunStats().fateRewrites || 0) + 1;
    addLog(`噬签改命：咬碎「${oldName}」，敌势改为「${newName}」。`, "important");
  } else if (!fallback && candidate) {
    getRunStats().fateRewriteKept = (getRunStats().fateRewriteKept || 0) + 1;
    addLog(`噬签改命：看过「${candidate.name}」后仍守原签「${oldName}」。`, "important");
  } else {
    addLog("噬签改命：此敌已无别签可换，满势照常结算。", "system-log");
  }
  ...
  render();
  if (!ended && game.status === "playing") setBattleMessage(`命签已定：${newName}。命势圆满随之结算。`);
  return true;
}
...
function requestFateRewrite() {
  ...
  if (benmingPassive("fate", 5)) {
    game.fateRewriteCandidate = {
      intentId,
      name: action.name || "未知命签",
      summary: getFateRewriteCandidateSummary(action),
    };
    game.inputLocked = true;
    addLog(`噬签改命·归墟：新签「${game.fateRewriteCandidate.name}」已显，只能在新旧两签中择一。`, "important");
    render();
    return true;
  }

  game.fateRewriteCandidate = { intentId, name: action.name || "未知命签", summary: getFateRewriteCandidateSummary(action) };
...
const UPDATE_LOG = [
  { v: "V0.9.37", title: "衔命虫双路线", notes: [
    "无名逆命者的衔命虫达到真形后，每局可在「三相织命」与「噬签改命」中选择一条互斥路线：前者改变命势生成，后者改变圆满使用时机。",
    "三相织命：连续织入攻击、护甲、辅助三类牌，第三类的出牌命势改为 +2；重复牌类会让三相重新起序，归墟后每回合首次重复可保住进度。",
    "噬签改命：每回合首次命势盈满时暂不结算，敌人意图区会出现醒目的「改签」按钮；归墟后可先看一张新签，再在新旧意图间二选一。",
...
function applyFateCardFlow(card) {
  ...
  if (pathId === "threeWeave") {
    ...
    if (result.completed) {
      ...
      addLog("三相织命：攻击、护甲、辅助三相合契，本张出牌命势改为 +2。", "important");
    } else if (result.graceConsumed) {
      addLog(`三相织命·归墟：${getCardFlowName(currentFlow)}之相虽重，命线本回合首次未断。`, "positive-log");
    } else if (result.repeated) {
      addLog(`三相织命：${getCardFlowName(currentFlow)}之相重复，命线由此重新织起。`, "system-log");
    }
...
function enterFateRewritePending() {
  ...
  game.player.fateMomentum = FATE_MOMENTUM_MAX;
  closeBattleCoach();
  spawnFloatText(dom.playerPortrait, "待圆满 · 改签", "fate-float");
  playFateFullEffect();
  addLog("噬签改命：命势盈满而未发。点击敌人意图上的「改签」后才会结算圆满。", "important");
  setBattleMessage("衔命虫已咬住命签：可点击敌人意图改签，或暂留满势。", "important");
  return true;
}
...
function resolveExistingFateAfterIntent() {
  ...
  const pathId = getActiveFateBenmingPath();
  if (pathId === "threeWeave") {
    addLog("三相织命：意图已显，承接的满势立即结算。", "important");
...
function endTurn() {
  ...
  if (game.fateRewritePending) {
    ...
    addLog("噬签改命：你暂不改签，满势随敌方行动留存。", "system-log");
  }
  game.inputLocked = true;
  render();
...
function showRunConclusion(cleared) {
  ...
  dom.runSummary.innerHTML = `
    ...
    ${(NMG_XIANGHUO_ENABLED && (cleared || __layer >= 2) && !progression.xianghuoHidePrompt) ? `<section class="run-summary-section run-sec-xianghuo">
...
function renderBuffs() {
  ...
  if (game.player.heroId === "fate") {
    ...
    if (fatePath === "threeWeave") {
      ...
      benmingPathBuff = {
        label: `三相织命：${triadNames.length ? triadNames.join(" · ") : "待起序"}`,
        title: BENMING_PATHS.fate.threeWeave.summary,
        className: "fate-path-status",
      };
    } else if (fatePath === "devourOmen") {
      benmingPathBuff = {
        label: game.fateRewritePending ? "噬签改命：待圆满 · 可改签" : "噬签改命：等待盈满",
...
function renderIntent() {
  ...
  if (rewriteReady) {
    dom.intentBox.title = showingChoice
      ? "新命签已显：必须采用新意图或保留原意图，随后结算圆满。"
      : "命势正在待圆满。点击醒目的「改签」按钮改写当前敌人意图并结算圆满。";
    dom.endTurnHint.textContent = showingChoice ? "请先决定新旧命签" : "可先改签，也可留势过回合";
  } else {
