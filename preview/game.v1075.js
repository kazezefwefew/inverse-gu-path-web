...
function getRouteMaxStep(state = null) {
  return (state ? getMingtuLegacyRoute(state) : OUTER_ROUTE_DEFINITION)?.maxLegacyStep || MAX_ROUTE_STEP;
}
...
function isCurrentBossRoute() {
  ...
  return runState.currentNode
    ? isBossRouteNode(runState.currentNode, runState)
    : isRouteBossSegment(runState.floor, runState);
}
...
function getEnemyIdForFloor(floor) {
  ...
  if (isRouteBossSegment(floor, runState)) return getMingtuLegacyRoute(runState)?.boss?.enemyId || "corpsepuppet";
  ...
  dom.mapDescription.textContent = __isL3Map
    ? (runState.currentRouteStep === 1
        ...
        : runState.currentRouteStep === 2
          ...
          : isRouteBossSegment(runState.currentRouteStep, runState)
            ? "绝域之主盘踞末路，破之则命途踏尽。"
