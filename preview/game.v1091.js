...
function showTowerHeartStructureCheckpoint() {
  ...
  dom.resultEyebrow.textContent = "命途塔 · 塔心";
  dom.resultTitle.textContent = scene?.name || "塔心";
  dom.resultDescription.textContent = "塔心命契已记住你停下的位置。";
  dom.resultTurns.textContent = "—";
  dom.resultHp.textContent = runState?.currentHp ?? "—";
  ...
  dom.resultPrimaryButton.classList.add("hidden");
  dom.resultSecondaryButton.textContent = "返回首页";
...
function resumeRunFromAutosave() {
  ...
  if (isMingtuTowerHeart(runState)) {
    ...
    showTowerHeartStructureCheckpoint();
    return true;
  }
...
const DEV_PANEL_GROUPS = [
  ...
  { title: "第三层（V0.9.8）", buttons: [
    ...
    ["模拟三层死亡", "simL3Death"], ["模拟三层通关", "simL3Clear"],
