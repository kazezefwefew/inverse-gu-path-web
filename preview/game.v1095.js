...
function showTowerHeartScene() {
  ...
  if (scene.id === "tower-heart-gate") {
    ...
    // boss / ending：E-2c4/c5 点亮前的结构占位 + 兼容结算退路
...
function renderTowerReflectionScene() {
  ...
  const rows = MUPAN_DEBT_TIE_ORDER.map((id) => {
    ...
    return `<div class="tower-debt-row${id === snapshot.primary.id ? " is-primary" : ""}${id === snapshot.secondary.id ? " is-secondary" : ""}">
