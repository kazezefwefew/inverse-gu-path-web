...
/* 角色结局（E-2c4 定结构：确认即章节通关；结局正文与残卷 E-2c5 展开）。 */
const TOWER_ENDING_LINES = Object.freeze({
  fate: "无名逆命者收刀而立。命途塔第一次，没有替任何人写完结局。",
  blood: "绛妄抹去唇边血色——这一次，笑意终于落到了实处。",
  poison: "青蟒收鳞敛息。毒债已清，蛇影自此行向有光处。",
  longevity: "朝暮吹熄灯芯，又亲手把它点亮。这一次，火由自己添。",
});
function renderTowerEndingScene() {
  const heroLine = TOWER_ENDING_LINES[runState.heroId] || "你走出盘心，塔外天光未定。";
  dom.towerHeartBody.textContent = `盘心的碎屑仍在空中缓缓旋落。\n${heroLine}\n（结局正文将随后续更新展开。）`;
  ...
  dom.towerHeartFoot.textContent = "确认后进行通关结算：难度、天印、印记、道行与资源在此一次性发放。";
}
