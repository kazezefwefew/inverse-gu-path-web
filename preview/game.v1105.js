...
function renderTowerReflectionScene() {
  ...
  dom.towerHeartBody.textContent = "万命母盘缓缓转动，六页命债账逐页翻开——你此行欠下的，都记在这里。";
  ...
  dom.towerHeartFoot.textContent = `主签「${snapshot.primary.sealName}」 · 副签「${snapshot.secondary.sealName}」——由本局行迹推得，读档重算不变；应战后不可改选。`;
}
function confirmTowerReflection() {
  ...
  addJourneyLog(`命债照见已毕：主签「${snapshot.primary.sealName}」、副签「${snapshot.secondary.sealName}」入盘待启。`, "important");
  saveRunStateToStorage();
  showTowerHeartScene();
}
...
/* E-2c4 已拆除塔心内「兼容结算桥」：正式链路 gate→整备→终问→照见→母盘战→角色结局→cleared 已全线贯通。
 * 入口选择页的「依旧例结算」（legacyFinal 通道）暂保留一批作为难度安全阀，E-2c5 总验收后移除。 */
...
function renderTowerBossScene() {
  ...
  dom.towerHeartBody.textContent = `账页已合。盘心之上，两支命签徐徐成形——\n主签「${primaryName}」 · 副签「${secondaryName}」。\n它不预知未来，只把你已经付出的代价，写成你的死法。`;
...
function sealMupanCard(instanceId) {
  ...
  playMupanSealFeedback("burn", sealId);
  hideRemovePickerOverlay();
  render();
  return true;
}
...
function getMupanVfxDuration(type, context = getMupanVfxContext()) {
  ...
  const desktop = { entrance: 1250, phase2: 1080, phase3: 1180, broken: 1600 };
  const mobile = { entrance: 820, phase2: 720, phase3: 780, broken: 1050 };
  return (context.mobile ? mobile : desktop)[type] || (context.mobile ? 520 : 680);
}

function getActiveMupanSealId(state = game?.mupan) {
  const seal = state?.core?.activeSeal;
  return seal?.id || seal?.sealIds?.find((id) => !seal.completedSealIds?.includes(id)) || seal?.sealIds?.[0] || "fate";
}
...
function playMupanVfx(type, options = {}) {
  if (!isMupanBattle() || !effectsAllowed()) {
    options.onComplete?.();
    return 0;
  }
...
function playMupanSealFeedback(kind, sealId = getActiveMupanSealId()) {
...
function endTurn() {
  ...
  if (isMupanBattle() && game.mupan.core.activeSeal?.status === "active") {
    const sealId = getActiveMupanSealId();
    ...
    if (game.mupan.stats.behaviorBreaks > before.stats.behaviorBreaks) {
      ...
      playMupanSealFeedback("break", sealId);
    }
  }
  game.inputLocked = true;
  render();
...
function applyMupanFailureAtSettlement(action) {
  ...
  const failedSealId = getActiveMupanSealId();
  const failedBefore = game.mupan.stats.failedSeals;
  const result = resolveMupanBattleFailure(game.mupan, { actionId: action.id, balance: ENEMY_BALANCE.mupan });
  game.mupan = result.state;
  if (game.mupan.stats.failedSeals > failedBefore) playMupanSealFeedback("fail", failedSealId);
...
function resolveEnemyTurn() {
  ...
  if (action.kind === "charge") {
    ...
    if (isMupanBattle()) {
      const armorSealId = getActiveMupanSealId();
      ...
      if (game.mupan.stats.behaviorBreaks > beforeBreaks) {
        ...
        playMupanSealFeedback("break", armorSealId);
      }
  ...
  if (isMupanBattle()) {
    ...
    const activeSealBefore = game.mupan.core.activeSeal;
    ...
    game.mupan.rewrittenAction = null;
    if (!activeSealBefore && game.mupan.core.activeSeal) playMupanSealFeedback("active", getActiveMupanSealId());
    render();
...
function finishBattle(victory) {
  if (isMupanBattle()) {
    ...
    if (game.isMupanTest) { finishMupanTestBattle(victory); return; }
    if (victory) { finishTowerMupanBattle(); return; }
    restoreMupanSealedCardsToBattle();
...
function snapshotDeathContext() {
  ...
  return {
    ...
    // E-2c4 §7.5：正式母盘战死时记录活动命签与阶段（死因分析可指向"死于哪支签"）
    mupanActiveSeal: (typeof isMupanBattle === "function" && isMupanBattle() && game.mupan.core.activeSeal?.id) || "",
...
function getRunStatsCopyText(outcome = runState?.status) {
  ...
  return [
    ...
    ...(cleared && isMingtuTowerHeart(runState) && runState.chapterProgress?.nodeId === "tower-heart-ending"
      ? [`角色结局：《${getTowerEnding().title}》${runState.heroId === "fate" && runState.benmingPath ? `（${getBenmingPathDisplayName(runState)}）` : ""}`]
      : []),
...
function showRunConclusion(outcome) {
  ...
  if (cleared) {
    ...
    // E-2c5b：塔心正门通关显示角色结局标题+短摘要；依旧例通道沿用原英雄尾声（E-2c5c 拆除该通道后此分支收敛）
...
function renderBuffs() {
  ...
  if (typeof updateStatusScrollAffordance === "function") updateStatusScrollAffordance(dom.buffList, buffs.length);
}
...
function refreshStatusScrollAffordances() {
  if (dom?.buffList) updateStatusScrollAffordance(dom.buffList, dom.buffList.querySelectorAll(".buff-tag").length);
...
function bindEvents() {
  ...
  // E-2c2 塔心场景动作委托：断契 / 整备 / 终问 / 照见 / 兼容结算 / 返回首页
...
const DEV_ACTIONS = {
  ...
  "mupanVfxPhase3": () => {
    if (!devRequireRun()) return;
    startMupanTestBattle({ mode: "normal", phase: 3, forceFinalSeal: true });
    dom.mupanLedgerOverlay?.classList.add("hidden"); game.inputLocked = false;
...
const DEV_PANEL_GROUPS = [
  ...
  { title: "最终战特效预览（E-2c5b.5）", buttons: [
    ["入场苏醒", "mupanVfxEntrance"], ["二阶段·阴阳并账", "mupanVfxPhase2"], ["三阶段·万命并书", "mupanVfxPhase3"],
