...
const GAME_VERSION = "V0.9.38 塔心终局";
...
function cacheDom() {
  [
    ...
    "guluOverlay", "guluCloseButton", "guluTitle", "guluBody", "guluOpenButton", "collectionBadge", "guluBadge", // V0.9.22/F-0 蛊庐与局外收藏
...
function getRunStatsHtml() {
  ...
  const stats = getRunStats();
  const guluCollection = typeof getGuluCollectionSummary === "function" ? getGuluCollectionSummary() : { entries: 0, hatched: 0, fed: 0 };
  const baseItems = [
    ...
    ["蛊庐藏册", `${guluCollection.entries} 种 · 孵化 ${guluCollection.hatched} · 投喂 ${guluCollection.fed}`],
  ];
  const mechanicItems = [
...
function getRunStatsCopyText(outcome = runState?.status) {
  ...
  return [
    ...
    `主动失血：${stats.selfHpLost}`,
    `主动耗寿：${stats.lifespanSpent}`,
    `蛊庐藏册：收录 ${guluCollection.entries} 种 / 累计孵化 ${guluCollection.hatched} / 已投喂 ${guluCollection.fed}`,
    ...(stats.mupan ? [
