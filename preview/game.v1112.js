...
function cacheDom() {
  [
    "startScreen", "mobileOrientationOverlay", "mobileLogButton", "mobileAudioToggle", "mobileAudioClose", "heroChoices", "benmingPathSection", "benmingPathTitle", "benmingPathHint", "benmingPathChoices", "relicChoices", "modeChoices", "advancedCardPreview", "startBattleButton", "runProgress", "trialModeHint",
...
function bindEvents() {
  ...
  dom.hand.addEventListener("click", (event) => {
    ...
    const card = game?.hand?.find((entry) => entry.instanceId === id);
    if (document.body.classList.contains("mobile-combat-safe") || getBloodAtonementPlan(card, false).eligible) showCardPreview(id);
    else playCard(id);
  });
