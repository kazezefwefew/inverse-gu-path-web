...
function cacheDom() {
  [
    ...
    "battleBackgroundSlot", "intentBox", "intentIcon", "intentName", "intentDescription", "fateRewriteButton", "fateRewriteChoice", "mupanSealPanel", "enemyPower", "enemyStatusList", "arenaKicker",
    ...
    "hand", "selectedCardActions", "selectedCardName", "selectedCardDetailButton", "selectedCardPlayButton", "castDisplay", "castGlyph", "castName", "resultOverlay", "resultSeal",
...
function updateMobileViewportState() {
  ...
  document.body.classList.toggle("start-prep-active", startOpen);
    ...
    return `<article class="hero-fate-card tone-${id} ${selected ? "selected" : ""}">
      <button class="hero-fate-select" type="button" data-hero-id="${id}" aria-pressed="${selected}" title="选择${hero.name}">
        ...
        <span class="hero-fate-copy">
          ...
          <span class="hero-fate-stats">生命 ${hero.maxHp} · 真元 ${hero.energy} · 寿元 ${hero.lifespan}</span>
          <span class="hero-fate-passive" title="${escapeAttribute(`${hero.passiveName}：${hero.passive}`)}"><b>${hero.passiveName}</b>：${hero.passive}</span>
        </span>
        ${mark}${guMark}
      </button>
