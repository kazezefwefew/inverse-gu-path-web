...
function isCompactAudioViewport() {
  return window.matchMedia?.("(min-width: 901px) and (max-width: 1300px)")?.matches || false;
}
...
function isMobileLandscapeSafe() {
  ...
  return landscape && window.innerHeight <= 720 && window.innerWidth <= 1280;
}
...
function updateMobileViewportState() {
  ...
  const compactAudio = isCompactAudioViewport();
...
function syncTitleSceneLive() {
  const scene = document.getElementById("titleScene");
  if (scene) scene.classList.toggle("ts-live", Boolean(effectsEnabled));
}
