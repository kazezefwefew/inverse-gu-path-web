...
const GAME_VERSION = "V0.9.39.1 视口修复";
...
function resolveAppViewportSample(previous, next, finalAttempt = false) {
  ...
  return {
    commit: finalAttempt && settled ? next : null,
    retry: finalAttempt && !settled,
...
function sampleAppViewport(finalAttempt = false, syncGeneration = appViewportSyncGeneration) {
  ...
  const resolution = resolveAppViewportSample(pendingAppViewportSample, sample, finalAttempt);
...
function createRunState() {
  ...
  const nextRun = {
    ...
    backlashCount: 0,
    stableCount: 0,
    runStats: { ...createRunStats(), benmingPath: getBenmingPathDefinition(progression.selectedHeroId, benmingPath)?.name || "未启用" },
