...
const GAME_VERSION = "V0.9.40.1 视口恢复热修";
...
/* FUNNEL-1：教程 5→3 页——首页只留"选人+一句核心"，真形路线等后期内容移至第 3 页；
 * 各行字符串原样保留（多处门禁按 includes 断言教程文案），战斗细节由战内 coach 提示按行为触发。 */
const TUTORIAL_PAGES = Object.freeze([
  ...
  },
  {
    ...
    lines: [
      ...
      "赤茧蛊达到真形后，绛妄每局可在缝煞成茧（先非血道、后自损血道）与裂茧代偿（主动用 3 血煞抵自损）中选择一条路线。",
      "蜕鳞蛊达到真形后，青蟒每局可在逆鳞后毒（先攻击、后施毒追 2 层）与蜕鳞借毒（剥敌 4 毒换 6 护甲）中选择一条路线。",
      "三相织命：依次打出攻击、护甲、辅助，第三类额外获得命势；凑齐前重复类型会重新起算。",
      "噬签改命：每回合第一次命势圆满时，可点击「改签」改换敌人准备使用的技能。",
      "铺垫：先打非血道牌，为缝煞准备；代偿：在卡牌确认页主动用 3 血煞抵掉最多 3 点自损。",
      "追毒：先攻击后施毒可追加毒层；借毒：敌人要攻击时可点按钮剥毒换护甲。",
      "普通战斗后可获得新蛊和炼蛊材料。",
      ...
      "塔外的万蛊录记录世界知识；蛊庐藏册只记录你亲手孵化、随行与投喂过的蛊虫。",
    ],
  },
]);
...
function cacheDom() {
  [
    ...
    "tutorialOpenButton", "tutorialResetButton", "tutorialDrillButton", "tutorialDrillFromTutorial", "loreOpenButton", "trialSettingsButton", "settingsOpenButton", "homeBackupButton", "recordingModeToggle", "tutorialOverlay", "tutorialCloseButton", "tutorialTitle", "tutorialBody",
    ...
    "guluOverlay", "guluCloseButton", "guluTitle", "guluBody", "guluOpenButton", "collectionBadge", "guluBadge", "guluActionConfirm", "guluActionConfirmClose", "guluActionConfirmTitle", "guluActionConfirmText", "guluActionConfirmCancel", "guluActionConfirmOk", // V0.9.22/F-0/P71 蛊庐与局外收藏
    "battleCoach", "battleCoachClose", "keywordTooltip",
    "mapScreen", "mapRoute", "mapHint", "mapDescription", "mapProgress", "mapStatus", "mapNotice", "mapSelectionPanel", "mapSelectionKind", "mapSelectionName", "mapSelectionRisk", "mapSelectionReward", "mapNodeConfirmButton", "mapTransition", "mapTransitionText", "mapGuStones", "mapDeckButton", "topGuStone",
    "deckViewButton", "resultDeckButton", "resultStatsButton", "resultLoreButton", "resultFeedbackButton", "feedbackCopyFallback", "deckOverlay", "deckCloseButton", "deckLoreButton", "deckTabs", "deckStatsButton", "deckSummary", "deckStats", "deckMaterials", "deckRelics", "deckMarks", "deckList", "deckCardDetail", "deckPrevPage", "deckNextPage", "deckPageLabel",
    ...
    "battleBackgroundSlot", "battleIntentRegion", "intentBox", "intentIcon", "intentName", "intentDescription", "intentSummary", "intentCollapseButton", "fateRewriteButton", "fateRewriteChoice", "poisonBorrowButton", "mupanSealPanel", "enemyPower", "enemyStatusList", "arenaKicker",
...
function measureAppViewport(viewportWindow = window, viewportDocument = document) {
  ...
  if (visualWidth > 0 && visualHeight > 0 && Math.abs(scale - 1) <= 0.05) {
    return { width: visualWidth, height: visualHeight, scale, source: "visualViewport" };
  }
  ...
  if (clientWidth > 0 && clientHeight > 0) {
    return { width: clientWidth, height: clientHeight, scale, source: "clientViewport" };
  }
  return {
    width: Math.max(0, Number(viewportWindow?.innerWidth) || 0),
    height: Math.max(0, Number(viewportWindow?.innerHeight) || 0),
    scale,
...
function isAppViewportSamplePlausible(sample, lastGood = null) {
  ...
  const comparableWidth = width >= goodWidth * 0.7 && width <= goodWidth * 1.3;
  const heightCollapsed = height < goodHeight * APP_VIEWPORT_COLLAPSE_HEIGHT_RATIO;
  return !(sameOrientation && comparableWidth && heightCollapsed);
}
...
function resolveAppViewportSample(previous, next, finalAttempt = false, lastGood = null) {
  ...
  return {
    commit: finalAttempt && settled && plausible ? next : null,
    retry: finalAttempt && (!settled || !plausible),
...
function sampleAppViewport(finalAttempt = false, syncGeneration = appViewportSyncGeneration) {
  ...
  const resolution = resolveAppViewportSample(pendingAppViewportSample, sample, finalAttempt, lastGoodAppViewport);
...
function getAppViewportSnapshot() {
  return lastGoodAppViewport || measureAppViewport();
}
...
function getAppViewportDebugText() {
  ...
  return `视口诊断：inner ${window.innerWidth}×${window.innerHeight} / client ${root.clientWidth}×${root.clientHeight} / visual ${visualText} / root ${Math.round(rootRect.width)}×${Math.round(rootRect.height)} / stable ${stableText} / css ${cssHeight} / ${orientation} / ${document.visibilityState}`;
}
...
function setHandCollapsed(collapsed) {
  ...
  if (dom.handCollapseToggle) {
    dom.handCollapseToggle.setAttribute("aria-expanded", String(!next));
    dom.handCollapseToggle.setAttribute("aria-label", next ? "展开手牌" : "收起手牌");
  }
}
...
function applyMobileViewportState() {
  ...
  document.documentElement.classList.toggle("start-prep-lock", startOpen);
...
function createRunState() {
  ...
  const mingtuContract = (contractDef && contractDef.implemented && isContractUnlocked(contractDef.id)) ? contractDef.id : null;
...
function syncPrepStepUi() {
  ...
  const pathEligible = isBenmingPathRequiredForTitle();
  if (!pathEligible && activePrepStep === "path") activePrepStep = "relic";
  dom.prepPathTab?.classList.toggle("hidden", !pathEligible);
  ...
  dom.prepStepTabs?.querySelectorAll("[data-prep-step]").forEach((tab) => {
    ...
    visibleStepNumber += 1;
    const number = tab.querySelector("b");
    if (number) number.textContent = ["一", "二", "三", "四"][visibleStepNumber - 1] || String(visibleStepNumber);
  });
}

function setPrepStep(step, { focus = true } = {}) {
  const allowed = new Set(["hero", "path", "relic", "mode"]);
  let next = allowed.has(step) ? step : "hero";
  if (next === "path" && !isBenmingPathRequiredForTitle()) next = "relic";
  activePrepStep = next;
  syncPrepStepUi();
  if (focus) {
...
function renderBenmingPathSelection() {
  ...
  dom.prepPathTab?.classList.toggle("hidden", !eligible);
  if (!eligible) {
...
function renderContractSelection() {
  ...
  const unlockedIds = getUnlockedContractIds();
  ...
  if (!selectedDef || !selectedDef.implemented || !isContractUnlocked(selectedDef.id)) progression.selectedContract = null;
  ...
  dom.contractChoices.innerHTML = CONTRACT_IDS.map((id) => {
    ...
    const unlocked = Boolean(loadContractStore().unlocked[id]);
    const signable = unlocked && def.implemented;
    if (!signable) {
      const stateText = unlocked ? "契文将至 · 下版开签" : `未解锁 · ${def.unlockHint}`;
...
function ensureBenmingPathSelected() {
  ...
  if (dom.runProgress) {
    dom.runProgress.textContent = `${BENMING_GU[heroId]?.name || "本命蛊"}已至真形，请先选择${names}。`;
    dom.runProgress.classList.remove("hidden");
  }
  dom.benmingPathSection?.classList.add("needs-choice");
  setPrepStep("path", { focus: false });
...
function renderTitleScreen() {
  ...
  updateHomeBackupDue(); // FUNNEL-1：超7天未备份则首页「备」印挂红点
...
function triggerLongevityVoice(event) {
...
function checkLongevityLowLife(beforeHp) {
  if (!game?.player || game.player.heroId !== "longevity") return;
  if (beforeHp > game.player.maxHp * 0.3 && game.player.hp <= game.player.maxHp * 0.3) triggerLongevityVoice("lowlife");
}
...
function renderMapSelection() {
  ...
  if (dom.mapSelectionName) dom.mapSelectionName.textContent = available ? node.name : "尚未择路";
  ...
  if (dom.mapNodeConfirmButton) {
    ...
    dom.mapNodeConfirmButton.textContent = available ? `踏入 · ${node.name}` : "踏入";
  }
...
function renderMapScreen() {
  ...
  dom.mapHint.textContent = currentStep <= getRouteMaxStep()
    ? "选择发亮节点继续；灰暗岔路本局不再开启。"
    : "命途已尽，等待结算。";
  if (dom.mapStatus) dom.mapStatus.innerHTML = `<span><em>当前</em><strong>${stepText}</strong></span><span><em>种子</em><strong>${escGu(runState.trialSeed || "无")}</strong></span><span><em>蛊石</em><strong>${runState.guStones}</strong></span>`;
  if (dom.mapProgress) {
...
function startFloorBattle() {
  ...
  recordBattleStarted();
  if (isBossNode) triggerLongevityVoice("boss");
  else if (currentNode?.type === "elite" || isDefyNode) triggerLongevityVoice("elite");
  else if (currentNode?.type === "battle") triggerLongevityVoice("battle");
  runState.rewardResolved = false;
...
/* FUNNEL-1 存档保险提级：记录最近备份时间；通关或超 7 天未备份即在结算页/首页备印上提醒。 */
...
function updateHomeBackupDue() {
  dom.homeBackupButton?.classList.toggle("is-due", isSaveExportDue());
}
...
function startTutorialDrill() {
  ...
  progression.selectedHeroId = "fate";
  progression.selectedBenmingPath = (typeof BENMING_PATHS !== "undefined" && BENMING_PATHS.fate
    && getEffectiveBenmingStage("fate") >= 3) ? "threeWeave" : null;
...
function finishTutorialDrill(victory) {
  ...
  renderTitleScreen();
  showRiteOverlay({
    tone: won ? "gold" : "blood", eyebrow: "教学演武", seal: "习",
    title: won ? "演武礼成" : "演武暂歇",
    text: won
      ? "交替出牌攒命势、圆满兑真元、见蓄力则打断——入塔的门道你已握住。\n真正的命途塔里，每一步都作数。"
      : "木人无怨，演武不作数。\n记住：交替出牌攒命势，见蓄力先打断。",
    hint: "点击任意处 · 回首页",
  });
}
...
function updateTutorialDrillCoach() {
  ...
  const m = game.player.fateMomentum || 0;
  const action = game.enemy?.intent ? getCurrentEnemyAction() : null;
  let step;
  if (action?.kind === "charge" && action.interruptThreshold) {
    step = `木人在蓄力——本回合对它打出 ${action.interruptThreshold} 点伤害就能打断；打不出就用铁皮蛊叠护甲硬接。`;
  } else if (!game.drillPlayedAny) {
    step = "点一张「攻击蛊」打出去（比如月刃蛊）。";
  } else if (m < 1) {
    step = "换一种类型再打一张（护甲或辅助）——类型交替，命势才会 +1。";
  } else if (m < FATE_MOMENTUM_MAX) {
    step = `继续交替出牌，把命势攒到 ${FATE_MOMENTUM_MAX}（现在 ${m}）；真元不够就结束回合。`;
  } else {
    step = "命势圆满会自动兑现：真元 +1、抽 1 张——乘胜追击，击败木人！";
  }
  panel.innerHTML = "<strong>教学演武</strong><p></p><button type=\"button\" id=\"drillQuitButton\">退出演武</button>";
  panel.querySelector("p").textContent = step;
  panel.classList.remove("hidden");
}

const COACH_TIP_STORE_KEY = "nmg.coachTips.v1";
function showCoachTip(key, text) {
  let seen = {};
  ...
  toast.innerHTML = "<strong>入塔提示</strong><p></p>";
...
function openMupanLedger() {
  ...
  game.inputLocked = true;
  dom.mupanLedgerLead.textContent = `母盘已从本局行迹锁定${debts.primary.name}与${debts.secondary.name}。破签，或被命签追加惩罚。`;
...
function detectLowEndDevice() {
  try {
    if ((window.NMG_PLATFORM || "web") !== "web") return true; // TapTap 等容器一律省电起步
...
function initPerfMode() {
  ...
  if (!stored && isPerfLite()) {
    showCoachTip("perfLiteAuto", "检测到设备性能有限，已自动开启省电模式（画面装饰减量，玩法不变）；可在设置里改为全效。");
  }
...
const UPDATE_LOG = [
  ...
  { v: "V0.9.39", title: "蜕鳞蛊双路线 · 血月塔影", notes: [
    "青蟒的蜕鳞蛊达到真形后，每局可在「逆鳞后毒」与「蜕鳞借毒」中选择一条互斥路线，取代旧的开局施毒。",
...
function settlePoisonBorrowedScaleAfterEnemyAction(action, attackLifeDamage) {
  ...
  addLog(`蜕鳞借毒·归墟：这一击未伤生命，返还敌人 ${plan.returned} 层旧毒。`, "positive-log");
...
function losePlayerHealth(amount) {
  ...
  checkLongevityLowLife(beforeHp);
...
function spendLifespan(amount, sourceName) {
  ...
  if (before > game.player.lifespan) triggerLongevityVoice("burn");
...
function applyMupanFailureAtSettlement(action) {
  ...
  checkLongevityLowLife(hpBefore);
...
function finishBattle(victory) {
  if (game?.tutorialDrill) { finishTutorialDrill(victory); return; } // 教学演武：不进奖励与结算管线
  if (isMupanBattle()) {
  ...
  if (victory) {
    if (currentNode?.type === "boss") triggerLongevityVoice("bossWin");
    else triggerLongevityVoice("victory");
    window.AudioManager?.playSfx?.("victory", { volumeScale: 0.5 });
    playVictoryEffect();
...
function getRunStatsHtml() {
  ...
  const stats = getRunStats();
  const guluCollection = typeof getGuluCollectionSummary === "function" ? getGuluCollectionSummary() : { entries: 0, hatched: 0, fed: 0, released: 0 };
  const baseItems = [
    ["结算结果", getRunOutcomeLabel()],
    ["试炼模式", getTrialModeInfo(runState.trialMode).name],
    ["本命路线", getBenmingPathDisplayName(runState)],
    ["命途契", (typeof getActiveContract === "function" && getActiveContract(runState)?.name) || "未签"],
    ...
    ["蛊庐藏册", `${guluCollection.entries} 种 · 孵化 ${guluCollection.hatched} · 投喂 ${guluCollection.fed} · 归野 ${guluCollection.released}`],
  ];
  const mechanicItems = [
    ...
    ["缝煞触发 / 少失生命", `${stats.bloodStitchTriggers || 0} / ${stats.bloodStitchHpSaved || 0}`],
    ["代偿次数 / 耗煞 / 少失生命", `${stats.bloodAtonementUses || 0} / ${stats.bloodAtonementSpent || 0} / ${stats.bloodAtonementHpSaved || 0}`],
    ["逆鳞追毒 / 额外毒层", `${stats.poisonAfterstrikeTriggers || 0} / ${stats.poisonAfterstrikeAdded || 0}`],
    ["借毒次数 / 剥毒 / 得甲 / 返毒", `${stats.poisonBorrowedScaleUses || 0} / ${stats.poisonBorrowedScalePoisonSpent || 0} / ${stats.poisonBorrowedScaleArmorGained || 0} / ${stats.poisonBorrowedScaleReturns || 0}`],
...
function getRunStatsCopyText(outcome = runState?.status) {
  ...
  return [
    ...
    `满势留签回合：${stats.fateRewriteWaitTurns || 0}`,
    `缝煞触发 / 少失生命：${stats.bloodStitchTriggers || 0} / ${stats.bloodStitchHpSaved || 0}`,
    `裂茧代偿次数 / 耗煞 / 少失生命：${stats.bloodAtonementUses || 0} / ${stats.bloodAtonementSpent || 0} / ${stats.bloodAtonementHpSaved || 0}`,
    `逆鳞追毒 / 额外毒层：${stats.poisonAfterstrikeTriggers || 0} / ${stats.poisonAfterstrikeAdded || 0}`,
    `蜕鳞借毒次数 / 剥毒 / 得甲 / 归墟返毒：${stats.poisonBorrowedScaleUses || 0} / ${stats.poisonBorrowedScalePoisonSpent || 0} / ${stats.poisonBorrowedScaleArmorGained || 0} / ${stats.poisonBorrowedScaleReturns || 0}`,
    `总防御：${stats.armorGained}`,
    `总治疗：${stats.healing}`,
    `主动失血：${stats.selfHpLost}`,
    `主动耗寿：${stats.lifespanSpent}`,
    `蛊庐藏册：收录 ${guluCollection.entries} 种 / 累计孵化 ${guluCollection.hatched} / 已投喂 ${guluCollection.fed} / 已归野 ${guluCollection.released}`,
    ...(stats.mupan ? [
...
function showRunConclusion(outcome) {
  ...
  if (cleared) triggerLongevityVoice("ending");
  const withdrawn = normalized === "withdrawn";
  const dead = normalized === "dead";
  ...
  dom.runSummary.innerHTML = `
    ...
    ${(__benmingGain || __carryTaken.length) ? `<section class="run-summary-section run-sec-harvest">
      <h4>跨局收获</h4>
...
function renderBuffs() {
  ...
  if (game.player.heroId === "fate") {
    ...
    if (bloodPath === "bloodStitch") {
      ...
      benmingPathBuff = { label: "赤茧蛊：旧规则·破茧吮煞", title: "此老续局沿用更新前的真形与归墟规则。", className: "blood-path-status" };
    }
    ...
    if (poisonPath === "poisonAfterstrike") {
      ...
      benmingPathBuff = { label: "蜕鳞蛊：旧规则·宿毒入局", title: "此老续局沿用更新前的真形与归墟规则。", className: "poison-path-status" };
    }
  }
...
function renderDeckOverlay() {
  ...
  dom.deckStats.innerHTML = getRunStatsHtml();
...
function setIntentCollapsed(collapsed) {
  intentCollapsed = Boolean(collapsed);
...
function renderHand() {
  ...
  dom.hand.innerHTML = game.hand.map((card) => {
    ...
    return `
      <button class="card ${card.type} category-${card.category} ${blockClass} ${upgradeClass} ${card.mutated ? "is-mutated" : ""} ${card.damaged ? "is-damaged" : ""} ${card.skewed ? "is-skewed" : ""} ${selectedHandCardId === card.instanceId ? "is-selected" : ""}" type="button"
...
function bindEvents() {
  ...
  dom.heroChoices.addEventListener("click", (event) => {
    ...
    if (choice.dataset.heroId === "longevity") triggerLongevityVoice("select");
    renderTitleScreen();
  });
  ...
  dom.guluOverlay?.addEventListener("click", (event) => {
    ...
    if (guluTab) {
      playGuluClick();
      guluActiveTab = guluTab.dataset.guluTab === "collection" ? "collection" : "home";
      renderGulu();
      return;
    }
  ...
  // FUNNEL-1：备份动作抽为 performSaveExport，供设置页/结算页/首页备印共用并记录最近备份时间
  dom.settingsSaveExport?.addEventListener("click", async () => {
    playUiSfx();
    await performSaveExport(dom.settingsSaveExport, "备份存档（复制并下载）");
  });
  dom.homeBackupButton?.addEventListener("click", async () => {
    playUiSfx();
    await performSaveExport(dom.homeBackupButton, "备");
  });
  ...
  dom.tutorialDrillButton?.addEventListener("click", enterTutorialDrill);
