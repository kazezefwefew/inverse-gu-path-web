...
const GAME_VERSION = "V0.9.42 蛊有其面";
...
function renderContractSelection() {
  ...
  if (!selectedDef || !selectedDef.implemented || !contractStore.unlocked[selectedDef.id]) progression.selectedContract = null;
...
function getRewardGlyphHtml(key, glyph) {
  ...
  return art
    ? `<span class="reward-card-glyph has-art"><img src="${art}" alt="" decoding="async"></span>`
  ...
  dom.hand.innerHTML = game.hand.map((card) => {
    ...
    return `
      <button class="card ${card.type} category-${card.category} ${blockClass} ${upgradeClass} ${card.mutated ? "is-mutated" : ""} ${card.damaged ? "is-damaged" : ""} ${card.skewed ? "is-skewed" : ""} ${selectedHandCardId === card.instanceId ? "is-selected" : ""} ${getCardArtImage(card.key) ? "card-with-art" : ""}" type="button"
        ...
        <div class="card-art ${getCardArtImage(card.key) ? "has-art" : ""}" aria-hidden="true">${getCardArtImage(card.key) ? `<img src="${getCardArtImage(card.key)}" alt="" decoding="async">` : card.art}</div>
