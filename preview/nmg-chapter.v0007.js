...
function makeLegacyNodes(routeKey, prefix) {
  const specs = prefix === ""
    ? [
        ...
        // V0.9.51 段数 6→9：新增三段，节点 id 全为新增，老 id 一个未改（老档按 id 定位，见 check-route-expansion-migration）
        [6, [["normal-5", "battle"], ["chance-3", "event"]]],
        [7, [["shop-3", "shop"], ["elite-2", "elite"], ["rest-3", "rest"]]],
        [8, [["normal-6", "battle"], ["chance-4", "event"]]],
      ]
    : [
        ...
        // V0.9.51 段数 6→9：同上，仅新增
        [6, [[`${prefix}-6-a`, "battle"], [`${prefix}-6-b`, "battle"]]],
        [7, [[`${prefix}-7-a`, "shop"], [`${prefix}-7-b`, "elite"], [`${prefix}-7-c`, "rest"]]],
        [8, [[`${prefix}-8-a`, "battle"], [`${prefix}-8-b`, "event"]]],
      ];
...
function makeRoute({ id, legacyId, name, legacyDisplayName, actId, legacyLayer, prefix, normalPool, eliteId, boss }) {
  ...
  const mapAnchors = Array.from({ length: 9 }, (_, index) => ({ // V0.9.51 段数 6→9
    id: `map-${id}-step-${index + 1}`,
    legacyStep: index + 1,
    role: "map",
  }));
  const lastNodes = nodes.filter((node) => node.legacyStep === 8); // V0.9.51：末段由 5 变 8
  ...
  return {
    ...
    actId,
    legacyLayer,
    maxLegacyStep: 9, // V0.9.51 段数 6→9（Boss 由第 6 段移至第 9 段）
    normalPool,
    eliteId,
    nodes,
    mapAnchors,
...
const MINGTU_TOWER_CHAPTER = deepFreezeChapterValue((() => {
  ...
  const outerBoss = {
    ...
    legacyStep: 9, // V0.9.51：Boss 随扩容移至末段
...
function migrateLegacyMingtuChapterProgress(run = {}) {
  ...
  /* V0.9.51 段数 6→9 后的旧档保护：Boss 的 legacyStep 由 6 变 9，旧档里 currentRouteStep=6 的
   * 「已打完 Boss 但节点已空」坏档不再与 boss.legacyStep 相等，清理条件会失配，
   * 导致 boss-1 残留在 completedNodes——玩家走到新第 9 段时 Boss 会被判为已完成而跳过。
   * 故改为直接按「completedNodes 里是否含 Boss 节点」判定，与段号脱钩。 */
  if (!canonicalNode && Array.isArray(run.completedNodes)) {
    const bossIds = new Set(route.boss.legacyNodeIds);
    if (run.completedNodes.some((nodeId) => bossIds.has(nodeId))) {
