...
(function createCloudSaveModule(global) {
  ...
  let state = Object.freeze({ status: "idle", message: "云存档尚未检查", lastSyncAt: 0, error: "", reloadRequired: false });

  function publish(status, message, extra = {}) {
    state = Object.freeze({ ...state, status, message, error: "", reloadRequired: false, ...extra });
  ...
  async function requestSync(reason = "manual") {
    try {
      // 云档已经落盘、旧内存正等待重载时，任何手动/轮询/生命周期同步都必须让路。
      if (state.reloadRequired) return state;
  ...
  async function bootstrapInternal() {
    ...
    try {
      ...
      if (!hasCloudGameplayData(local.payload)) {
        ...
        return publish("restored", "已从云端恢复上次进度", { lastSyncAt: cloud.payload.savedAt, reloadRequired: true });
      }
      ...
      if (lastSyncedHash && local.payload.hash === lastSyncedHash) {
        const regressions = typeof findCloudProgressRegressions === "function"
          ? findCloudProgressRegressions(local.payload, cloud.payload)
          : [];
        if (regressions.length) {
          pendingConflict = { local, cloud, localText: JSON.stringify(local.payload), regressions };
          const labels = { benming: "本命道行", tian: "十重天", gulu: "成蛊转数" };
          const detail = regressions.slice(0, 3).map((item) => {
            const after = item.after < 0 ? "缺失" : item.after;
            return `${labels[item.kind] || "长期进度"} ${item.before}→${after}`;
          }).join("；");
          const conflict = publish("conflict", "云端进度可能较旧，已阻止自动覆盖", { error: `${detail}${regressions.length > 3 ? "；另有更多差异" : ""}` });
          startDirtyPolling();
          return conflict;
        }
        ...
        setStored(META_HASH_KEY, cloud.payload.hash);
        pendingConflict = null;
        return publish("restored", "已从云端恢复较新的进度", { lastSyncAt: cloud.payload.savedAt, reloadRequired: true });
      }
  ...
  async function resolveConflict(choice) {
    try {
      ...
      if (choice === "cloud") {
        ...
        pendingConflict = null;
        const restored = publish("restored", "已恢复云端进度，本机冲突档已留作备份", { lastSyncAt: chosen.cloud.payload.savedAt, reloadRequired: true });
