...
function renderBenmingCodex() {
  ...
  const cards = Object.entries(BENMING_GU).map(([heroId, gu]) => {
    ...
    const pathOverview = BENMING_PATHS[heroId] ? `<div class="benming-path-overview ${info.stage < 3 ? "is-locked" : ""}">
      <p><strong>真形双路线</strong><span>${legacyRun ? "当前老续局沿用旧规则" : (info.stage >= 3 ? "每局开局二择一" : "真形后解锁")}</span></p>
      ${Object.values(BENMING_PATHS[heroId]).map((path) => `<section class="benming-path-entry ${selectedPathId === path.id ? "is-active" : ""}">
        <b>${path.glyph}</b><div><strong>${path.name}${selectedPathId === path.id ? " · 当前局" : ""}</strong><span>${path.summary}</span><small>归墟：${path.guixu}</small></div>
      </section>`).join("")}
    </div>` : "";
  ...
  return '<p class="wangulu-counter">本命蛊 · 跨局养成 · 五修道行合计 ' + totalDao + '（局末自动结算，休整节点可饲蛊）</p>'
