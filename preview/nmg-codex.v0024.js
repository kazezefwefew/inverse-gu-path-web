...
function openWanGuLu() {
  ...
  selectedLoreId = "";
  if (getCodexClaimableCount() > 0) wanGuLuState.tab = "task";
  markCodexHubSeen();
  renderWanGuLu();
  wanGuLuEl.classList.remove("hidden");
  if (typeof showCoachTip === "function") {
    showCoachTip("firstWanGuLu", "万蛊录记录已经见过的蛊虫、敌怪与世界知识；蛊庐藏册则记录你亲手孵化和养成过的成蛊。两处用途不同。", { forceToast: true, outOfRunTitle: true });
  }
  ...
  if (claimBtn) {
    ...
    if (result && result.ok) {
      ...
      if (typeof enqueueOutgameReceipt === "function") {
        const items = [{ glyph: "印", name: `图鉴印记 · ${result.taskName || "未名"}`, amount: 1, detail: "已入万蛊录" }];
        if ((result.scrip | 0) > 0) items.push({ glyph: "契", name: "蛊钱", amount: result.scrip | 0, detail: "已入百蛊市" });
        enqueueOutgameReceipt(
          { source: "万蛊录 · 图鉴任务", title: "图鉴印记已领取", tone: "gold", items, summary: "收藏进度与实际蛊钱均已写入存档。" },
...
function renderContractCodexSection() {
  ...
  return '<div class="codex-contract-section">'
    + '<h4 class="codex-contract-title">命途契 · 司命人的暗契</h4>'
    + '<p class="codex-contract-sub">完成与玩法同构的达成即得契；下一局整备时可择一签契改写规则，代价明码。</p>'
    + rows
    + '</div>';
}

/* 图鉴任务列表：条件 + 奖励预告 + 真实进度 + 醒目「预埋」徽章。进度满也不发奖。
 * ※ 注意：本函数存在同名后定义（图鉴印记领取版）覆盖——此处为死代码，改行为请去后一处。 */
...
function renderCodexOverview() {
  ...
  return '<p class="wangulu-counter">蛊道进境 · 残卷所窥，不过初篇</p>'
    ...
    + '</div>'
    + '<p class="codex-overview-verse">完成图鉴收集成就可领取蛊钱。<br>蛊钱只在塔外百蛊市流通，回蛊庐兑换奇物。</p>';
}
...
function renderCodexTasks() {
  const cards = CODEX_TASKS.map((task) => {
    ...
    return '<div class="' + cardCls + '">'
      ...
      + (function () {
        // V0.9.40 QS-1a：绑定契的任务追加契指向行（契名与条件读 CONTRACTS，单一真源不写死数字）。
