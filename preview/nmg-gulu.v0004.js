...
function renderGulu() {
  ...
  const altarPathId = currentRunHero
    ...
    : (heroId === "fate" ? getBenmingPathDefinition("fate", progression.selectedBenmingPath)?.id : null);
  ...
  const altarPaths = heroId === "fate" && bi.stage >= 3 ? `<div class="benming-path-overview gulu-benming-paths ${injured && getEffectiveBenmingStage(heroId) < 3 ? "is-suppressed" : ""}">
    <p><strong>真形双路线</strong><span>${legacyBenmingRun ? "当前老续局沿用旧规则" : (injured && getEffectiveBenmingStage(heroId) < 3 ? "静养降阶，路线暂失效" : (altarPathId ? `本局：${getBenmingPathDefinition("fate", altarPathId)?.name}` : "新局入塔前二择一"))}</span></p>
    ${Object.values(BENMING_PATHS.fate).map((path) => `<section class="benming-path-entry ${altarPathId === path.id ? "is-active" : ""}">
