...
function getMaterialFitText(id) {
  ...
  if (id === "boneCrystal") return "破甲蛊、蚀甲蛊";
...
function isMaterialMatched(entry, materialId) {
  ...
  if (materialId === "boneCrystal") return entry.key === "armorBreaker" || entry.key === "armorMeltPoison" || typeName.includes("破甲") || typeName.includes("蚀甲");
...
function openFurnace() {
  ...
  if (furnaceScene) {
    ...
    furnaceScene.classList.remove("is-heating", "is-forging");
  }
...
function confirmFurnaceUpgrade() {
  ...
  const furnaceScene = dom.furnacePanel?.querySelector(".furnace-scene");
  furnaceScene?.classList.add("is-heating", "is-forging");
  setTimeout(() => furnaceScene?.classList.remove("is-forging"), 900);
