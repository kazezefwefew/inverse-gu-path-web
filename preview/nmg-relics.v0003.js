...
function buildCombatRelicRailMarkup() {
  ...
  const activeChip = activeRelic
    ? `<span class="combat-relic-chip active-relic-chip" data-relic-id="__active" role="button" tabindex="0" title="本命·${activeRelic.name}：${activeRelic.description}" data-keyword="${activeRelic.name}" aria-label="本命遗物·${activeRelic.name}：${activeRelic.description}"><b>${activeRelic.glyph}</b></span>`
    : "";
  const ordinaryChips = ids.map((id) => {
    const r = ORDINARY_RELICS[id];
    return `<span class="combat-relic-chip relic-${r.tone || "gold"}" data-relic-id="${id}" role="button" tabindex="0" title="${r.name}：${r.description}" data-keyword="${r.name}" aria-label="遗物·${r.name}：${r.description}"><b>${r.glyph}</b></span>`;
  }).join("");
  return activeChip + ordinaryChips;
}
